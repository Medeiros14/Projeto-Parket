...
(function () {
  ...
  function renderLista() {
    ...
    pendentesCache.forEach((item) => {
      ...
      const sims = Array.isArray(item.simulacoes) ? item.simulacoes : (item.simulacao ? [item.simulacao] : []);
      ...
      const multiBadge = sims.length > 1
        ? ' · <span style="color:#A855F7;font-weight:700">' + sims.length + ' propostas</span>' +
          (decididasCount > 0 ? ' <span style="color:rgba(255,255,255,0.45);font-size:10px">(' + decididasCount + ' já decidida' + (decididasCount === 1 ? '' : 's') + ')</span>' : '')
        : '';
      const sharedBtns =
      ...
      let simsHTML = '';
      if (sims.length === 0) {
        simsHTML = '<div style="font-size:11px;color:#F59E0B">⚠ Sem proposta vinculada</div>' +
          '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:8px">' + sharedBtns + '</div>';
      } else {
        simsHTML = '<div style="display:flex;flex-direction:column;gap:8px">';
        sims.forEach((s) => {
          ...
          const decTag = decidida
            ? (decidida.decisao === 'aprovado'
                ? '<span style="font-size:10px;padding:2px 8px;border-radius:10px;background:rgba(16,185,129,0.18);color:#10B981;font-weight:700;text-transform:uppercase">✓ Aprovada</span>'
                : '<span style="font-size:10px;padding:2px 8px;border-radius:10px;background:rgba(239,68,68,0.18);color:#EF4444;font-weight:700;text-transform:uppercase">✕ Rejeitada</span>')
            : '<span style="font-size:10px;padding:2px 8px;border-radius:10px;background:rgba(245,158,11,0.18);color:#F59E0B;font-weight:700;text-transform:uppercase">⏳ Pendente</span>';
          simsHTML +=
            '<div style="background:rgba(255,255,255,0.025);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:10px 12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">' +
              '<div style="flex:1;min-width:160px">' +
                '<div style="font-size:12px;color:#fff;font-weight:700">Proposta #' + numero + ' ' + decTag + '</div>' +
                (decidida && decidida.motivo
                  ? '<div style="font-size:10px;color:rgba(255,255,255,0.5);margin-top:2px">Motivo: ' + decidida.motivo.slice(0,120) + (decidida.motivo.length > 120 ? '…' : '') + '</div>'
                  : '') +
              '</div>' +
              (link
                ? '<button data-act="ver" data-id="' + item.card.id + '" data-sim="' + s.id + '" data-numero="' + numero + '" style="padding:6px 12px;border-radius:5px;background:rgba(212,168,83,0.18);border:1px solid rgba(212,168,83,0.5);color:#D4A853;cursor:pointer;font-size:10px;font-weight:700;text-transform:uppercase">Ver</button>'
                : '') +
              (decidida
                ? '<button data-act="duvida-sim" data-id="' + item.card.id + '" data-sim="' + s.id + '" data-numero="' + numero + '" title="Comentar sobre essa proposta no chat do orçamentista" style="padding:6px 12px;border-radius:5px;background:rgba(59,130,246,0.18);border:1px solid rgba(59,130,246,0.5);color:#3B82F6;cursor:pointer;font-size:10px;font-weight:700;text-transform:uppercase">💬 Comentar</button>'
                : '<button data-act="duvida-sim" data-id="' + item.card.id + '" data-sim="' + s.id + '" data-numero="' + numero + '" title="Tirar dúvida sobre essa proposta" style="padding:6px 12px;border-radius:5px;background:rgba(59,130,246,0.18);border:1px solid rgba(59,130,246,0.5);color:#3B82F6;cursor:pointer;font-size:10px;font-weight:700;text-transform:uppercase">💬 Dúvida</button>' +
                  '<button data-act="rejeitar-sim" data-id="' + item.card.id + '" data-sim="' + s.id + '" data-numero="' + numero + '" style="padding:6px 12px;border-radius:5px;background:rgba(239,68,68,0.18);border:1px solid rgba(239,68,68,0.5);color:#EF4444;cursor:pointer;font-size:10px;font-weight:700;text-transform:uppercase">✕ Rejeitar</button>' +
                  '<button data-act="aprovar-sim" data-id="' + item.card.id + '" data-sim="' + s.id + '" data-numero="' + numero + '" style="padding:6px 12px;border-radius:5px;background:rgba(16,185,129,0.22);border:1px solid rgba(16,185,129,0.55);color:#10B981;cursor:pointer;font-size:10px;font-weight:700;text-transform:uppercase">✓ Aprovar</button>') +
            '</div>';
        });
        simsHTML += '</div>';
        simsHTML += '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:12px;border-top:1px solid rgba(255,255,255,0.05);padding-top:10px">' + sharedBtns + '</div>';
      }
        ...
        '<div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:10px">' +
          '<div style="flex:1;min-width:0">' +
            ...
            '<div style="font-size:11px;color:rgba(255,255,255,0.55)">' + obraTag +
              'Há ' + fmtSince(item.card.updated_at || item.card.created_at) + histTag + multiBadge +
            '</div>' +
          '</div>' +
        '</div>' +
    ...
    body.querySelectorAll("button[data-act]").forEach((btn) => {
      btn.onclick = async () => {
        ...
        const simNumero = btn.dataset.numero;
        if (act === "ver") {
          const item = pendentesCache.find(p => p.card.id === cardId);
          if (item) renderProposta(item, simId);
          return;
        }
        if (act === "conversa") {
          const item = pendentesCache.find(p => p.card.id === cardId);
        ...
        if (act === "duvida") {
          // Card-level dúvida (legacy, sem número)
          const item = pendentesCache.find(p => p.card.id === cardId);
        ...
        if (act === "duvida-sim") {
          const item = pendentesCache.find(p => p.card.id === cardId);
          await abrirModalComentario({ tipo: "duvida", cardId, item, btn, simId, simNumero });
          return;
        }
        if (act === "rejeitar-sim") {
          const item = pendentesCache.find(p => p.card.id === cardId);
          await abrirModalComentario({ tipo: "rejeicao", cardId, item, btn, simId, simNumero });
          return;
        }
        if (act === "aprovar-sim") {
          await acaoDecisaoSim("aprovar", cardId, simId, btn);
          return;
        }
        // Legacy (card-level) — mantém pra compat
        if (act === "rejeitar") {
          const item = pendentesCache.find(p => p.card.id === cardId);
  ...
  function renderProposta(item, simId) {
    const body = document.getElementById("pkt-overlay-body");
    if (!body) return;
    // Resolve qual sim e qual link
    const sims = Array.isArray(item.simulacoes) ? item.simulacoes : (item.simulacao ? [item.simulacao] : []);
    const simAlvo = (simId && sims.find(s => s.id === simId)) || sims.find(s => !s.decisao) || sims[0] || null;
    const link = (simAlvo && simAlvo.link) || item.proposta_link;
    if (!link) return;
    const targetSimId = simAlvo ? simAlvo.id : null;
    const decidida = simAlvo && simAlvo.decisao;
    const numero = simAlvo ? (simAlvo.numero || (simAlvo.id ? simAlvo.id.slice(0,8) : "—")) : "—";
    viewState = "proposta";
    propostaAberta = item;
    ...
    const propTag = sims.length > 1 ? ' <span style="font-size:10px;padding:2px 6px;border-radius:8px;background:rgba(168,85,247,0.18);color:#A855F7;font-weight:700;text-transform:uppercase">Proposta #' + numero + '</span>' : '';
    const decTag = decidida
      ? (decidida.decisao === 'aprovado'
          ? ' <span style="font-size:10px;padding:2px 8px;border-radius:10px;background:rgba(16,185,129,0.18);color:#10B981;font-weight:700;text-transform:uppercase">✓ Já aprovada</span>'
          : ' <span style="font-size:10px;padding:2px 8px;border-radius:10px;background:rgba(239,68,68,0.18);color:#EF4444;font-weight:700;text-transform:uppercase">✕ Já rejeitada</span>')
      : '';
    ...
    body.innerHTML =
      '<div style="background:#0A0A0A;border-bottom:1px solid rgba(255,255,255,0.08);padding:10px 18px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-shrink:0">' +
        '<div style="flex:1;min-width:0"><div style="font-size:14px;font-weight:700;color:#fff;line-height:1.2">' + (item.cliente || "—") + propTag + decTag + '</div></div>' +
        '<a href="' + link + '" target="_blank" rel="noopener" title="Abrir em nova aba" style="padding:6px 10px;border-radius:6px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.12);color:rgba(255,255,255,0.6);text-decoration:none;font-size:11px;font-weight:700">↗</a>' +
      ...
      '</div>' +
      '<iframe src="' + link + '" style="flex:1;width:100%;border:0;background:#fff" title="Proposta"></iframe>' +
      '<div style="background:#0A0A0A;border-top:2px solid rgba(255,255,255,0.1);padding:12px 18px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-shrink:0;flex-wrap:wrap">' +
        ...
        '<div style="display:flex;gap:8px;align-items:center;flex-shrink:0">' +
          ...
          '<button data-act="duvida-sim" data-id="' + item.card.id + '" data-sim="' + (targetSimId || '') + '" data-numero="' + numero + '" style="padding:8px 18px;border-radius:6px;background:rgba(59,130,246,0.2);border:1px solid rgba(59,130,246,0.55);color:#3B82F6;cursor:pointer;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em">💬 ' + (decidida ? 'Comentar' : 'Dúvida') + '</button>' +
          (decidida
            ? ''
            : '<button data-act="rejeitar-sim" data-id="' + item.card.id + '" data-sim="' + (targetSimId || '') + '" data-numero="' + numero + '" style="padding:8px 18px;border-radius:6px;background:rgba(239,68,68,0.2);border:1px solid rgba(239,68,68,0.55);color:#EF4444;cursor:pointer;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em">✕ Reprovar</button>' +
              '<button data-act="aprovar-sim" data-id="' + item.card.id + '" data-sim="' + (targetSimId || '') + '" data-numero="' + numero + '" style="padding:8px 18px;border-radius:6px;background:rgba(16,185,129,0.25);border:1px solid rgba(16,185,129,0.6);color:#10B981;cursor:pointer;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em">✓ Aprovar</button>') +
        '</div>' +
      '</div>';
    ...
    body.querySelectorAll("button[data-act]").forEach((btn) => {
      btn.onclick = async () => {
        const act = btn.dataset.act;
        const sId = btn.dataset.sim;
        const sNum = btn.dataset.numero;
        if (act === "duvida-sim") {
          await abrirModalComentario({ tipo: "duvida", cardId: btn.dataset.id, item, btn, simId: sId, simNumero: sNum });
          return;
        }
        if (act === "duvida") {
        ...
        if (act === "rejeitar-sim") {
          await abrirModalComentario({ tipo: "rejeicao", cardId: btn.dataset.id, item, btn, simId: sId, simNumero: sNum });
          return;
        }
        if (act === "aprovar-sim") {
          await acaoDecisaoSim("aprovar", btn.dataset.id, sId, btn);
          return;
        }
        // Legacy
        if (act === "rejeitar") {
  ...
  // ════════════════════════════════════════════════════════════════════
  function abrirModalComentario({ tipo, cardId, item, btn, simId, simNumero }) {
    return new Promise((resolve) => {
      ...
      const titulo = isRejeicao
        ? ("Justificar Rejeição" + (propLabel ? " — " + propLabel : ""))
        : ("Comentar com Orçamentista" + (propLabel ? " — " + propLabel : ""));
      const subtitulo = isRejeicao
        ...
        : ("Faça uma pergunta ou comente. Vai pro chat do card com a tag " + (propLabel || "do card") + ", o orçamentista vê quando abrir.");
      ...
      const btnLabel = isRejeicao ? "✕ Confirmar Rejeição" : "💬 Enviar Comentário";
      ...
      wrap.querySelector("#pkt-ceo-coment-ok").onclick = async () => {
        ...
        if (isRejeicao) {
          if (simId) {
            await acaoDecisaoSim("rejeitar", cardId, simId, btn || okBtn, v);
          } else {
            await acaoDecisao("rejeitar", cardId, btn || okBtn, v);
          }
        } else {
          await postarComentario(cardId, v, "duvida", item, simNumero);
  ...
  // ════════════════════════════════════════════════════════════════════
  async function postarComentario(cardId, texto, tipo, item, simNumero) {
    ...
    const prefix = tipo === "rejeicao"
      ...
      : (propTag ? `[DÚVIDA CEO — ${propTag}]` : "[DÚVIDA CEO]");
    const body = {
      context_id: cardId,
    ...
    if (tipo === "duvida") {
      ...
      const propLine = simNumero ? ` — Proposta #${simNumero}` : "";
      try {
        await waNotifyGroup(`💬 *DÚVIDA do CEO* — ${cli}${propLine}\n\nOrçamentista: ${orc}\n\n*Pergunta:*\n${texto}\n\nResponder pelo chat do card no Space.`, orcPhone);
  ...
  // Multi-sim: decide UMA proposta (não move o card até todas serem decididas).
  async function acaoDecisaoSim(act, cardId, simId, btn, motivoOverride) {
    const email = getUserEmail();
    const nome = getUserNome();
    if (!email) { alert("Login não detectado"); return; }
    if (!simId) { alert("Sim ID ausente"); return; }
    const reqBody = { email, nome };
    if (act === "rejeitar") {
      const m = motivoOverride && motivoOverride.trim();
      if (!m) { alert("Justificativa obrigatória."); return; }
      reqBody.motivo = m;
    } else {
    ...
    btn.disabled = true; const originalTxt = btn.textContent; btn.textContent = "…";
    try {
      const r = await fetch(`${API}/api/orcamento/aprovacao/por-simulacao/${simId}/${act === "aprovar" ? "aprovar" : "rejeitar"}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(reqBody),
      });
      ...
      // WhatsApp opcional (igual fluxo card-level)
      try {
        const cli = itemReg && itemReg.cliente ? itemReg.cliente : "Cliente";
        const orc = itemReg && itemReg.orcamentista ? itemReg.orcamentista : "—";
        const vd = itemReg && itemReg.vendedor ? itemReg.vendedor : "—";
        const orcPhone = await lookupOrcamentistaPhone(orc);
        const sim = itemReg ? (itemReg.simulacoes||[]).find(s => s.id === simId) : null;
        const propLabel = sim ? `#${sim.numero || simId.slice(0,8)}` : "";
        if (act === "aprovar") {
          await waNotifyGroup(`✅ *CEO APROVOU* — ${cli} — Proposta ${propLabel}\n\nOrçamentista: ${orc}\nVendedor: ${vd}`, orcPhone);
        } else {
          await waNotifyGroup(`❌ *CEO REJEITOU* — ${cli} — Proposta ${propLabel}\n\nOrçamentista: ${orc}\nVendedor: ${vd}\n\n*Motivo:*\n${reqBody.motivo}`, orcPhone);
        }
      } catch(_we) { console.warn("[wa-decisao-sim]", _we); }

      if (resp.card_movido) {
        // Card movido: tira da lista
        pendentesCache = pendentesCache.filter((p) => p.card.id !== cardId);
        lastCount = pendentesCache.length;
        ensureTab();
        const body = document.getElementById("pkt-overlay-body");
        ...
        // Só atualiza essa sim no cache; re-renderiza
        if (itemReg) {
          const sim = (itemReg.simulacoes || []).find(s => s.id === simId);
          if (sim) {
            sim.decisao = {
              decisao: act === "aprovar" ? "aprovado" : "rejeitado",
              motivo: reqBody.motivo || null,
              aprovado_por_nome: nome,
              decidido_em: new Date().toISOString(),
            };
          }
          itemReg.pendentes_count = (itemReg.simulacoes || []).filter(s => !s.decisao).length;
          itemReg.decididas_count = (itemReg.simulacoes || []).length - itemReg.pendentes_count;
        }
        // Se estávamos na view de proposta, volta pra lista
        const body = document.getElementById("pkt-overlay-body");
        if (body && viewState === "proposta") {
          body.style.padding = "24px";
          body.style.maxWidth = "1100px";
          body.style.gap = "14px";
        }
