/** Almoxarifado — port do ERP legado (Apps Script): Posição de Estoque, Entrada Manual,
 *  Saída de Material (protocolo) e Importar XML NF-e. Dados em compras_estoque_mov/compras_notas/
 *  compras_saidas/compras_contas_pagar (Supabase Cloud). */
...
import {
  fmtBRL, fmtQtd, fmtData, hojeISO, printHtml, printProtocolo,
  fetchProdutos, ensureProduto, fetchProjetos, proximoProtocolo, proximoProtocoloEntrada,
  categoriaInsumo, categoriaPorNcm,
  type Movimento, type Produto,
} from "../lib/erp";
import { FileUp, Pencil, Plus, Printer, Search, Trash2, X } from "lucide-react";

type Aba = "posicao" | "entrada" | "saida" | "hist-entrada" | "historico" | "xml";
export type Deposito = "marcenaria" | "instalacao" | "marcenaria-curitiba";

const DEPOSITOS: { id: Deposito; label: string; resp: string }[] = [
  { id: "marcenaria", label: "Marcenaria", resp: "Ronaldo" },
  { id: "instalacao", label: "Instalação", resp: "Taiara" },
...
export default function Almoxarifado() {
  ...
  const [deposito, setDeposito] = useState<Deposito>("marcenaria");
  ...
  const ABAS: { id: Aba; label: string }[] = [
    ...
    { id: "hist-entrada", label: "Histórico de Entradas" },
  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      ...
      {aba !== "historico" && aba !== "hist-entrada" && <div style={{ display: "flex", gap: 4, alignItems: "center", marginBottom: 20 }}>
      ...
      {aba === "hist-entrada" && <HistoricoEntradas st={st} t={t} />}
    ...
    <div>
      ...
      {editKey && (
        <EditarProduto st={st} t={t} movs={movsDe(editKey)}
          produto={produtos.find((p) => p.codigo === saldos.find((s) => s.key === editKey)?.codigo)}
...
function EditarProduto({ st, t, movs, produto, categoria, onClose, onSaved }: {
  st: St; t: any; movs: Movimento[]; produto?: Produto; categoria: string; onClose: () => void; onSaved: () => void;
}) {
  ...
  async function salvar() {
    ...
    try {
      ...
      const apelidosArr = Array.from(new Set(
        apelidos.split("\n").map((s) => s.trim().toUpperCase()).filter((s) => s && s !== nomeNovo)));
      const apelidosMudou = JSON.stringify(apelidosArr) !==
        JSON.stringify((produto?.apelidos || []).map((a) => a.trim().toUpperCase()));
      if (codigo && (nomeNovo !== nomeAntigo || cat !== categoria || apelidosMudou)) {
        await sb.from("compras_produtos").update({
          descricao: nomeNovo, categoria: cat, apelidos: apelidosArr,
          ...(cat !== categoria ? { categoria_manual: true } : {}),
        }).eq("codigo", codigo);
      }
      onSaved();
    } catch (err: any) {
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={st.lblSt}>Apelidos — outros nomes do mesmo material (um por linha)</label>
            <textarea value={apelidos} onChange={(e) => setApelidos(e.target.value)} rows={3}
                      placeholder={"Ex: o nome como vem na nota fiscal\nPARAFUSO PHILIPS 4X40 CX500"}
                      style={{ ...st.inpSt, resize: "vertical", fontFamily: "inherit" }} />
            <div style={{ fontSize: 10.5, color: t.textMuted, marginTop: 4 }}>
              Quando a NF ou a solicitação chega com um desses nomes, o sistema entende que é este produto
              (estoque consolida no nome principal e o Conferir estoque do card acha o saldo).
            </div>
            {!produto && (
              <div style={{ fontSize: 10.5, color: t.warning, marginTop: 4 }}>
                Este item não tem produto cadastrado (movimentos antigos sem código) — apelidos não podem ser salvos.
              </div>
            )}
          </div>
...
function Entrada({ st, t, deposito, setDeposito, produtos, projetos, fornecedores, onSaved }: {
  ...
  async function salvar() {
    ...
    setMsg("");
    try {
      // Documento do lançamento: nº da NF/recibo quando informado, senão um protocolo ENT-000123
      const docRef = documento.trim() || (await proximoProtocoloEntrada());
      ...
      for (const i of validos) {
        ...
        const prodM = cache.find((p) => p.codigo === codigo);
        movRows.push({
          data, produto_codigo: codigo, descricao: (prodM?.descricao || i.descricao).trim().toUpperCase(),
          tipo: "Entrada", quantidade: qtd, unidade: i.unidade || "UN",
          valor_unitario: vu, valor_total: qtd * vu,
          documento: docRef, fornecedor: fornecedor.trim() || null,
      ...
      imprimirProtocolo(docRef, validos);
  ...
  // Protocolo de entrada no modelo oficial (mesmo layout da saída)
  function imprimirProtocolo(docRef: string, its: ItemLinha[]) {
    printProtocolo({
      tipo: "entrada",
      numero: docRef,
      subtitulo: "Entrada por Digitação",
      dataEmissao: data,
      dataEstoque: new Date().toISOString(),
      deposito: DEPOSITOS.find((d) => d.id === deposito)?.label || deposito,
      centroCusto: projeto.trim() || "USO INTERNO",
      destinoLabel: "Fornecedor / Origem",
      destinoNome: fornecedor.trim() || "Não informado",
      destinoEndereco: documento.trim() ? `Documento fiscal: ${documento.trim()}` : null,
      natureza: "Entrada de material",
      observacoes: null,
      itens: its.map((i) => ({
        descricao: i.descricao.trim().toUpperCase(),
...
function Saida({ st, t, deposito, setDeposito, produtos, projetos }: { st: St; t: any; deposito: Deposito; setDeposito: (d: Deposito) => void; produtos: Produto[]; projetos: string[] }) {
  ...
  // O campo "Projeto de destino" é um só: quando o texto digitado bate com uma
  // obra do banco, a saída sai vinculada a ela (cliente + endereço no protocolo).
  const obraSel = useMemo(() => {
    const alvo = projeto.trim().toUpperCase();
    if (!alvo) return null;
    return obras.find((o) => o.cliente.trim().toUpperCase() === alvo) || null;
  }, [projeto, obras]);

  // Sugestões do campo: obras do banco primeiro, depois projetos já digitados
  // em saídas antigas que não correspondem a nenhuma obra.
  const opcoesProjeto = useMemo(() => {
    const nomes = obras.map((o) => o.cliente.trim()).filter(Boolean);
    const vistos = new Set(nomes.map((n) => n.toUpperCase()));
    for (const p of projetos) {
      const n = (p || "").trim();
      if (n && !vistos.has(n.toUpperCase())) { vistos.add(n.toUpperCase()); nomes.push(n); }
    }
    return nomes;
  }, [obras, projetos]);
  ...
  async function salvar() {
    ...
    try {
      ...
      const { data: saida, error: eSai } = await sb.from("compras_saidas").insert({
        ...
        cliente: obraSel?.cliente || null,
        endereco_cliente: obraSel?.localizacao || null,
  ...
  // Protocolo de saída no modelo oficial (mesmo layout da entrada)
  function imprimirProtocolo(protocolo: string, its: SaidaLinha[]) {
    printProtocolo({
      tipo: "saida",
      numero: protocolo,
      subtitulo: "Saída por Digitação",
      dataEmissao: data,
      dataEstoque: new Date().toISOString(),
      deposito: DEPOSITOS.find((d) => d.id === deposito)?.label || deposito,
      centroCusto: projeto.trim() || "USO INTERNO",
      destinoLabel: "Cliente destino",
      destinoNome: obraSel?.cliente || projeto.trim() || "USO INTERNO",
      destinoEndereco: obraSel?.localizacao || null,
      natureza: "Saída de material",
      observacoes: obs.trim() || null,
      itens: its.map((i) => ({
        descricao: i.descricao.trim().toUpperCase(),
        unidade: i.unidade || "UN",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18, maxWidth: 980 }}>
      <div style={st.painel}>
        <div style={{ display: "grid", gridTemplateColumns: "140px 190px 1fr", gap: 12, marginBottom: 12 }}>
          <div><label style={st.lblSt}>Data *</label>
            <input type="date" value={data} onChange={(e) => setData(e.target.value)} style={st.inpSt} /></div>
          ...
          {/* Campo único de projeto: a lista traz as obras do banco e os projetos
              já usados em saídas anteriores. Bateu com uma obra, a saída sai
              vinculada a ela (cliente e endereço entram no protocolo). */}
          <div><label style={st.lblSt}>Projeto de destino</label>
            ...
            <datalist id="dl-projetos-s">{opcoesProjeto.map((p) => <option key={p} value={p} />)}</datalist>
            <div style={{ marginTop: 4, fontSize: 10, color: obraSel ? t.accent : t.textMuted }}>
              {obraSel ? `Vinculado à obra: ${obraSel.cliente}${obraSel.localizacao ? ` · ${obraSel.localizacao}` : ""}` : "Sem obra vinculada (uso interno)"}
            </div></div>
        </div>
        <div>
...
type SaidaHist = {
  ...
  cliente: string | null; endereco_cliente: string | null;
...
function HistoricoSaidas({ st, t }: { st: St; t: any }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: saidas }, { data: movs }] = await Promise.all([
        sb.from("compras_saidas").select("id,protocolo,data_emissao,projeto,cliente,endereco_cliente,observacoes,criado_por")
  ...
  // Reimpressão usa exatamente o mesmo modelo da emissão
  function reimprimir(r: SaidaHist) {
    printProtocolo({
      ...
      dataEstoque: null,
      deposito: r.depositos.map(depLabel).join(", ") || "Não informado",
      centroCusto: r.projeto || "USO INTERNO",
      destinoLabel: "Cliente destino",
      destinoNome: r.cliente || r.projeto || "USO INTERNO",
      destinoEndereco: r.endereco_cliente || null,
      natureza: "Saída de material",
...
/* ─── HISTÓRICO DE ENTRADAS (relatório de todos os almoxarifados) ───────── */
type EntradaHist = {
  id: string; tipo: string; numero_nf: string | null; documento: string;
  data_entrada: string; fornecedor: string | null; projeto: string;
  valor_total: number; criado_por: string | null;
  depositos: string[];
  itens: { descricao: string; quantidade: number; unidade: string }[];
};

function HistoricoEntradas({ st, t }: { st: St; t: any }) {
  const [rows, setRows] = useState<EntradaHist[]>([]);
  const [loading, setLoading] = useState(true);
  const [busca, setBusca] = useState("");
  const [fDep, setFDep] = useState<"" | Deposito>("");
  const [fTipo, setFTipo] = useState<"" | "manual" | "xml">("");
  const [de, setDe] = useState("");
  const [ate, setAte] = useState("");

  const depLabel = (d: string) => DEPOSITOS.find((x) => x.id === d)?.label || d;

  // Entrada = nota (manual ou XML NF-e) + os movimentos que ela gerou no estoque
  useEffect(() => {
    let ativo = true;
    setLoading(true);
    (async () => {
      const [{ data: notas }, { data: movs }] = await Promise.all([
        sb.from("compras_notas").select("id,tipo,numero_nf,fornecedor_nome,projeto,data_entrada,valor_total,criado_por")
          .order("data_entrada", { ascending: false }).limit(500),
        sb.from("compras_estoque_mov").select("nota_id,documento,deposito,descricao,quantidade,unidade")
          .eq("tipo", "Entrada").in("deposito", DEPOSITOS.map((d) => d.id)).limit(20000),
      ]);
      if (!ativo) return;
      const porNota = new Map<string, any[]>();
      for (const m of (movs as any[]) || []) {
        if (!m.nota_id) continue;
        const a = porNota.get(m.nota_id) || []; a.push(m); porNota.set(m.nota_id, a);
      }
      const out: EntradaHist[] = [];
      for (const n of (notas as any[]) || []) {
        const its = porNota.get(n.id) || [];
        if (!its.length) continue;
        out.push({
          id: n.id, tipo: n.tipo, numero_nf: n.numero_nf,
          documento: its[0]?.documento || (n.numero_nf ? `NF ${n.numero_nf}` : "ENTRADA"),
          data_entrada: n.data_entrada, fornecedor: n.fornecedor_nome, projeto: n.projeto,
          valor_total: Number(n.valor_total) || 0, criado_por: n.criado_por,
          depositos: Array.from(new Set(its.map((m: any) => m.deposito).filter(Boolean))),
          itens: its.map((m: any) => ({ descricao: m.descricao, quantidade: Number(m.quantidade), unidade: m.unidade || "UN" })),
        });
      }
  ...
  const filtradas = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (de && r.data_entrada < de) return false;
      if (ate && r.data_entrada > ate) return false;
      if (!q) return true;
      return r.documento.toUpperCase().includes(q)
        || (r.numero_nf || "").toUpperCase().includes(q)
        || (r.fornecedor || "").toUpperCase().includes(q)
        || (r.projeto || "").toUpperCase().includes(q)
        || r.itens.some((i) => i.descricao.toUpperCase().includes(q));
    });
  }, [rows, busca, fDep, fTipo, de, ate]);

  // Protocolo de entrada no mesmo modelo da saída
  function reimprimir(r: EntradaHist) {
    printProtocolo({
      tipo: "entrada",
      numero: r.documento,
      subtitulo: r.tipo === "xml" ? "Entrada por NF-e" : "Entrada por Digitação",
      dataEmissao: r.data_entrada,
      dataEstoque: null,
      deposito: r.depositos.map(depLabel).join(", ") || "Não informado",
      centroCusto: r.projeto || "USO INTERNO",
      destinoLabel: "Fornecedor / Origem",
      destinoNome: r.fornecedor || "Não informado",
      destinoEndereco: r.numero_nf ? `Documento fiscal: NF ${r.numero_nf}` : null,
      natureza: "Entrada de material",
      observacoes: r.valor_total ? `Valor total da nota: ${fmtBRL(r.valor_total)}` : null,
      itens: r.itens,
    });
  }
  ...
  function imprimirRelatorio() {
    const linhas = filtradas.map((r) => r.itens.map((i, ix) => `
      <tr>${ix === 0 ? `<td rowspan="${r.itens.length}">${r.documento}</td><td rowspan="${r.itens.length}">${fmtData(r.data_entrada)}</td><td rowspan="${r.itens.length}">${r.depositos.map(depLabel).join(", ")}</td><td rowspan="${r.itens.length}">${r.fornecedor || "-"}</td><td rowspan="${r.itens.length}">${r.projeto}</td>` : ""}
      <td>${i.descricao}</td><td>${fmtQtd(i.quantidade)} ${i.unidade}</td></tr>`).join("")).join("");
    const periodo = de || ate ? ` · Período: ${de ? fmtData(de) : "…"} a ${ate ? fmtData(ate) : "…"}` : "";
    const totalValor = filtradas.reduce((s, r) => s + r.valor_total, 0);
    printHtml("Relatório de Entradas · Almoxarifados", `
      <p class="meta"><b>Depósito:</b> ${fDep ? depLabel(fDep) : "Todos"}${periodo} &nbsp;·&nbsp; <b>Entradas:</b> ${filtradas.length} &nbsp;·&nbsp; <b>Valor total:</b> ${fmtBRL(totalValor)}</p>
      <table><thead><tr><th>Documento</th><th>Data</th><th>Depósito</th><th>Fornecedor</th><th>Projeto</th><th>Material</th><th>Qtd</th></tr></thead><tbody>${linhas}</tbody></table>`);
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 14, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 240px", position: "relative" }}>
          <Search size={13} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: t.textMuted }} />
          <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar documento, NF, fornecedor, material…"
                 style={{ ...st.inpSt, paddingLeft: 30 }} />
        </div>
        <div><label style={st.lblSt}>Depósito</label>
          <select value={fDep} onChange={(e) => setFDep(e.target.value as any)} style={{ ...st.inpSt, width: 200, appearance: "auto" as any }}>
        ...
        <div><label style={st.lblSt}>Origem</label>
          <select value={fTipo} onChange={(e) => setFTipo(e.target.value as any)} style={{ ...st.inpSt, width: 150, appearance: "auto" as any }}>
      ...
      <div style={{ border: `1px solid ${t.border}`, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>
            <th style={st.thSt}>Documento</th><th style={st.thSt}>Data</th><th style={st.thSt}>Depósito</th>
            <th style={st.thSt}>Fornecedor</th><th style={st.thSt}>Projeto</th><th style={st.thSt}>Materiais</th>
            <th style={st.thSt}>Valor</th><th style={st.thSt}>Por</th><th style={{ ...st.thSt, width: 40 }} />
          </tr></thead>
          <tbody>
            {loading && <tr><td colSpan={9} style={{ ...st.tdSt, color: t.textMuted }}>Carregando…</td></tr>}
            {!loading && !filtradas.length && <tr><td colSpan={9} style={{ ...st.tdSt, color: t.textMuted }}>Nenhuma entrada encontrada.</td></tr>}
            {filtradas.map((r) => (
              <tr key={r.id}>
                <td style={{ ...st.tdSt, fontWeight: 600, whiteSpace: "nowrap" }}>
                  {r.documento}
                  <div style={{ fontSize: 10, color: t.textMuted, fontWeight: 400 }}>{r.tipo === "xml" ? "XML NF-e" : "Manual"}</div>
                </td>
                <td style={{ ...st.tdSt, whiteSpace: "nowrap" }}>{fmtData(r.data_entrada)}</td>
                <td style={st.tdSt}>{r.depositos.map(depLabel).join(", ")}</td>
                <td style={st.tdSt}>{r.fornecedor || "-"}</td>
                <td style={st.tdSt}>{r.projeto}</td>
                <td style={st.tdSt}>
                  {r.itens.map((i, ix) => (
                    <div key={ix} style={{ whiteSpace: "nowrap" }}>{i.descricao} · {fmtQtd(i.quantidade)} {i.unidade}</div>
                  ))}
                </td>
                <td style={{ ...st.tdSt, whiteSpace: "nowrap" }}>{fmtBRL(r.valor_total)}</td>
                <td style={{ ...st.tdSt, color: t.textMuted, fontSize: 11 }}>{r.criado_por || "-"}</td>
                <td style={st.tdSt}>
                  <button onClick={() => reimprimir(r)} title="Imprimir protocolo de entrada"
                          style={{ background: "transparent", border: "none", color: t.textMuted, cursor: "pointer" }}>
                    <Printer size={13} />
                  </button>
                </td>
      ...
      <div style={{ fontSize: 11, color: t.textMuted, marginTop: 8 }}>
        {filtradas.length} entrada{filtradas.length === 1 ? "" : "s"} · últimas 500, todos os almoxarifados deste app.
      </div>
    </div>
  );
}
...
function ImportXml({ st, t, deposito, setDeposito, produtos, projetos, onSaved }: {
  ...
  const [msg, setMsg] = useState("");
  const [cats, setCats] = useState<("MATERIA_PRIMA" | "FERRAGEM" | "EMBALAGEM")[]>([]);

  function sugestaoCat(desc: string, ncm?: string): "MATERIA_PRIMA" | "FERRAGEM" | "EMBALAGEM" {
    const up = (desc || "").trim().toUpperCase();
    const hit = produtos.find((p) => p.descricao.trim().toUpperCase() === up);
    const catHit = ["FERRAGEM", "MATERIA_PRIMA", "EMBALAGEM"].includes(hit?.categoria as any)
      ? (hit!.categoria as "MATERIA_PRIMA" | "FERRAGEM" | "EMBALAGEM") : null;
    // correção manual do usuário vale mais que qualquer leitura automática
    if (catHit && (hit as any).categoria_manual) return catHit;
    return categoriaPorNcm(ncm || "") || catHit || categoriaInsumo(desc);
  }

  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    setErro(""); setMsg(""); setNfe(null); setCats([]);
    ...
    try {
      ...
      setNfe(parsed);
      setCats(parsed.itens.map((i) => sugestaoCat(i.descricao, i.ncm)));
    } catch (err: any) {
  ...
  async function processar() {
    ...
    setMsg("");
    try {
      // dedup por chave de acesso — nota sem movimentos é vestígio de exclusão: limpa e segue
      if (nfe.chaveAcesso) {
        const { data: dup } = await sb.from("compras_notas").select("id").eq("chave_acesso", nfe.chaveAcesso).limit(1);
        if (dup?.length) {
          const { count } = await sb.from("compras_estoque_mov").select("id", { count: "exact", head: true }).eq("nota_id", dup[0].id);
          if (count) throw new Error("Esta NF-e já foi importada (chave de acesso duplicada).");
          await sb.from("compras_contas_pagar").delete().eq("nota_id", dup[0].id);
          await sb.from("compras_notas").delete().eq("id", dup[0].id);
        }
      }
      ...
      if (nfe.fornecedorCnpj) {
        ...
        const { data: forn } = await sb.from("compras_fornecedores").select("id,telefone,email,razao_social,obs")
          .or(`cnpj.eq.${nfe.fornecedorCnpj},cnpj.eq.${cnpjFmt}`).limit(1);
        if (!forn?.length) {
          // NOVO fornecedor — cria com todos os dados do emitente da NFe
          await sb.from("compras_fornecedores").insert({
            nome: nfe.fornecedorNome,
            razao_social: nfe.fornecedorRazao || nfe.fornecedorNome,
            cnpj: cnpjFmt,
            telefone: nfe.fornecedorTelefone || null,
            email: nfe.fornecedorEmail || null,
            obs: nfe.fornecedorEndereco ? `Endereço: ${nfe.fornecedorEndereco}` : null,
            tipo: "geral", categoria: "Geral", avaliacao: 0, entregas: 0, atrasos: 0, valor_k: 0, ativo: true,
          });
        } else {
          // EXISTE — só preenche campos vazios (não sobrescreve dados que Financeiro já cadastrou)
          const f = forn[0] as any;
          const patch: Record<string, any> = {};
          if (!f.telefone && nfe.fornecedorTelefone) patch.telefone = nfe.fornecedorTelefone;
          if (!f.email && nfe.fornecedorEmail) patch.email = nfe.fornecedorEmail;
          if (!f.razao_social && nfe.fornecedorRazao) patch.razao_social = nfe.fornecedorRazao;
          if (!f.obs && nfe.fornecedorEndereco) patch.obs = `Endereço: ${nfe.fornecedorEndereco}`;
          if (Object.keys(patch).length) await sb.from("compras_fornecedores").update(patch).eq("id", f.id);
        }
      }
      ...
      const cache = [...produtos];
      const movRows = [];
      for (let ix = 0; ix < nfe.itens.length; ix++) {
        const i = nfe.itens[ix];
        const codigo = await ensureProduto(i.descricao, i.unidade, cache, cats[ix] || null);
        // escolha do usuário prevalece; se divergiu da sugestão automática, fica marcada como manual
        const prod = cache.find((p) => p.codigo === codigo);
        const manual = !!cats[ix] && cats[ix] !== sugestaoCat(i.descricao, i.ncm);
        if (prod && cats[ix] && (prod.categoria !== cats[ix] || (manual && !(prod as any).categoria_manual))) {
          const upd: any = { categoria: cats[ix] };
          if (manual) upd.categoria_manual = true;
          await sb.from("compras_produtos").update(upd).eq("codigo", codigo);
          (prod as any).categoria = cats[ix];
          if (manual) (prod as any).categoria_manual = true;
        }
        movRows.push({
          // Se o nome da NF é apelido de um produto cadastrado, o estoque consolida no nome popular
          data: hojeISO(), produto_codigo: codigo, descricao: (prod?.descricao || i.descricao).trim().toUpperCase(),
          ...
          <div style={st.painel}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 11.5, color: t.textMuted }}>Definir tipo de todos:</span>
                <select value="" onChange={(e) => { const v = e.target.value as "MATERIA_PRIMA" | "FERRAGEM" | "EMBALAGEM"; if (v) setCats(nfe.itens.map(() => v)); }}
                  style={{ ...st.inpSt, width: "auto", padding: "4px 8px", appearance: "auto" as any }}>
                  <option value="">—</option>
                  <option value="MATERIA_PRIMA">Matéria prima</option>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr>
                <th style={st.thSt}>Cód. fornecedor</th><th style={st.thSt}>Descrição</th><th style={st.thSt}>Tipo</th><th style={st.thSt}>Qtd</th>
              ...
              <tbody>
                {nfe.itens.map((i, ix) => (
                  <tr key={ix}>
                    ...
                    <td style={st.tdSt}>
                      <select value={cats[ix] || "MATERIA_PRIMA"} onChange={(e) => setCats((p) => p.map((c, j) => j === ix ? e.target.value as any : c))}
                        title="Tipo de material" style={{ ...st.inpSt, width: "auto", padding: "3px 6px", appearance: "auto" as any }}>
                        <option value="MATERIA_PRIMA">Matéria prima</option>
                        <option value="FERRAGEM">Ferragem</option>
                        <option value="EMBALAGEM">Embalagem</option>
                      </select>
