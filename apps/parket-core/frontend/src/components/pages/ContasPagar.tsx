...
import { Loader2, AlertCircle, ArrowUpFromLine, Clock, CalendarDays, Wallet, Plus, Pencil, Undo2 } from "lucide-react";
...
const STATUS_OPCOES = ["todos", "previsto", "a_pagar", "atrasado", "pago", "conciliado", "cancelado"] as const;
...
export function ContasPagarPage() {
  ...
  const result = useMemo(() => {
    ...
    const empById = new Map(emp.data.map((e) => [e.id, e]));
    ...
    const isPago = (s: string) => s === "pago" || s === "conciliado";
    const rows = lan.data
    ...
    let totalProx30 = 0;
    for (const l of rows) {
      if (isPago(l.status) || l.status === "cancelado") continue;
    ...
    const filtered = rows.filter((l) => {
      const venc = new Date(l.data_vencimento + "T00:00:00");
      const aberto = !isPago(l.status) && l.status !== "cancelado";
    ...
    return { totalAPagar, totalAtrasado, totalProx7, totalProx30, filtered, paById, pcById, obById, empById, cbById };
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2"><ArrowUpFromLine size={18} className="text-parket-accent" /> Contas a pagar</h1>
          <p className="text-xs text-parket-textDim mt-1">Saídas previstas e em aberto</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <Stat icon={Wallet} label="Total a pagar" value={fmtBRL(result.totalAPagar)} accent />
        <Stat icon={AlertCircle} label="Em atraso" value={fmtBRL(result.totalAtrasado)} danger />
        <Stat icon={Clock} label="Vence em 7d" value={fmtBRL(result.totalProx7)} />
        <Stat icon={CalendarDays} label="Vence em 30d" value={fmtBRL(result.totalProx30)} />
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-parket-panelLight text-[10px] uppercase tracking-wider text-parket-textDim font-bold border-b border-parket-border">
              ...
              <th className="text-left px-3 py-2.5">Fornecedor</th>
          ...
          <tbody>
            ...
            {result.filtered.map((l) => {
              ...
              const fornecedor = l.parceiro_id ? result.paById.get(l.parceiro_id) : null;
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 font-semibold">{fornecedor?.nome || "—"}</td>
                  ...
                  <td className="px-3 py-2 text-right tabular-nums font-semibold">
                    ...
                    {baixado && l.valor_pago != null && Number(l.valor_pago) !== Number(l.valor) && (
                      <div className="text-[9px] text-parket-textDim">pago {fmtBRL(l.valor_pago)}</div>
                    )}
                  </td>
                  <td className="px-3 py-2 text-parket-textDim text-[11px]">
                    {contaBanco ? (
