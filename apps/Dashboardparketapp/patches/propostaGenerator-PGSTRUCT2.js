...
`).find(z);if(l){N=B(l);break}}if((C.toUpperCase()==="PAINEL"||C.toUpperCase()==="REVESTIMENTO")&&/LAMIN/i.test(N)){N=N.replace(/\s*\d+\/\d+\s*[×x]\s*\d+\s*MM?/gi,"").replace(/\s+/g," ").trim();}g+=`<tr>
      ...
      <td class="cat-l">${(function(_x){var _w=/^PAINEL$/i.test(C);var _r=(__cParts[2]||"").trim();var _isLam=_r && /\d+\/\d+\s*[×x]/i.test(_r);var _hasOtherAcab=/^\s*(RIPADO|TOBLERONE|MUXARABI|ASSOALHO|MACI[ÇC]O|MOLDURA|LACA|BLINDADA|REGUA|RÉGUA)\b/i.test(_x);var _name=(!_w||!_x||!_isLam||_hasOtherAcab)?_x:("LÂMINA "+_x);_name=_name.replace(/\s+MACI[ÇC][OA]\s*$/i,"").trim();var _hasDimPattern=_r&&/\d.{0,12}[×xX].{0,12}\d/.test(_r);var _showDim=_hasDimPattern;return _showDim?(_name+'<br><span class="cat-l-dim">'+_r+'</span>'):_name;})((C.toUpperCase()==="PORTA"?N.replace(/\b(?:DN150|RO82TOP|CIR|GERIS|ITALY\s+LINE|3D)\b/gi," ").replace(/\b(ASSOALHO|LACA|LAMINA|MOLDURA[_ ]VIDRO|MUXARABI|RIPADO|TOBLERONE)\b\s+(?=\S)/gi," ").replace(/\s+/g," ").trim().split(" ").filter(Boolean).filter((_v,_i,_a)=>_i===0||_v.toUpperCase()!==_a[_i-1].toUpperCase()).join(" ").replace(/_/g," "):((function(){var _t=N.replace(/_/g," "),_c=C.toUpperCase();if(_c==="MARCENARIA"){return _t.split(/\s*\|\s*/).map(function(s){return s.trim().toUpperCase()}).filter(Boolean).join(" / ");}var _isLam=(_c==="PAINEL"||_c==="REVESTIMENTO")&&_t&&!/LAMINA|RIPADO|TOBLERONE|DECK|MACI[ÇC]O|MOLDURA|MUXARABI|DEMOLI[ÇC][AÃ]O|MULTIBLOC|LACA|ASSOALHO|R[ÉE]GUA/i.test(_t);return _t.replace(/\s+/g," ").trim().split(" ").filter(Boolean).filter((_v,_i,_a)=>_i===0||_v.toUpperCase()!==_a[_i-1].toUpperCase()).join(" ")})())))}</td>
      <td class="cat-v"></td>
    </tr>`;let _recIdx=0,_ambSum=0,_ambCount=0,_subtotalAdded=false,_rowIdx=0,_ambMetragemSum=0,_ambPerdaPerc=10;t.forEach((d,l)=>{const _hdU=((d.descritivo||"").split("\n")[0]||"").trim().toUpperCase();const _isAmbItem=!/^INSTALA|^INSUMOS|^RECORTES/.test(_hdU);if(_isAmbItem){_ambSum+=(+d.valor||0);_ambCount++;var _amDesc=d.descritivo||"";var _amLns=_amDesc.split("\n");for(var _li=0;_li<_amLns.length;_li++){var _amPP=_amLns[_li].match(/Metragem\s+real(?:\s+total)?\s+[0-9]+(?:[.,][0-9]+)?\s*m²\s*\+\s*([0-9]+)\s*%\s*de\s+perda/i);if(_amPP){_ambPerdaPerc=parseFloat(_amPP[1])||_ambPerdaPerc;}var _amM=_amLns[_li].match(/^\s*Metragem\s+real(?:\s+total)?\s+([0-9]+(?:[.,][0-9]+)?)\s*m²/i);if(_amM){_ambMetragemSum+=parseFloat(_amM[1].replace(",","."));break;}_amM=_amLns[_li].match(/^\s*Metragem\s+total\s+com\s+perda\s+de\s+([0-9]+)\s*%\s*([0-9]+(?:[.,][0-9]+)?)\s*m²/i);if(_amM){_ambMetragemSum+=parseFloat(_amM[2].replace(",","."))/(1+parseFloat(_amM[1])/100);_ambPerdaPerc=parseFloat(_amM[1])||_ambPerdaPerc;break;}}}else if(!_subtotalAdded&&_ambCount>=2){g+=`<tr><td class="item-c"></td><td class="item-l">Metragem real total ${_ambMetragemSum.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}m² + ${_ambPerdaPerc}% de perda = ${Math.round(_ambMetragemSum*(1+_ambPerdaPerc/100)).toLocaleString("pt-BR")}m²</td><td class="item-v">${E(_ambSum)}</td></tr>`;_subtotalAdded=true;}const _isRec=(d.descritivo||"").toUpperCase().startsWith("RECORTES");const u=(d.descritivo||"").split(`
`).map(I=>I.trim()).filter(Boolean).filter(I=>!z(I)&&!/^__RECDATA__:/.test(I)&&!/^\s*\d+\s+unidade(s)?\s*$/i.test(I)&&!(/^\s*\d+\/\d+\s*[×x]\s*\d+\s*(MM)?\s*$/i.test(I))&&!(C.toUpperCase()==='MARCENARIA'&&(/^\s*\(inclui ferragens/i.test(I)||/^\s*Madeiras:/i.test(I)||/^\s*Metragem (real|total)/i.test(I)))).map(I=>I.replace(/[a-záéíóúç]+_[a-záéíóúç_]+/gi,W=>W.replace(/_/g," "))).map(function(I){var _hd=((d.descritivo||"").split("\n")[0]||"").trim().toUpperCase();if(/^INSUMOS|^RECORTES/.test(_hd)){var _insM=I.match(/^(\s*INSUMOS\s+DE\s+[A-ZÁÀÂÃÉÊÍÓÔÕÚÇ]+)\b/i);if(_insM)return _insM[1].toUpperCase();return I;}if(/^INSTALA/.test(_hd)){if(/^Metragem/i.test(I) && _ambMetragemSum>0){return "Metragem total "+Math.round(_ambMetragemSum)+"m²";}return I;}if(/^Metragem\s+real\s+[0-9.,]+\s*m²\s*\+\s*[0-9]+\s*%\s*de\s+perda/i.test(I))return I;var _mN=I.match(/^Metragem\s+total\s+com\s+perda\s+de\s+([0-9]+)\s*%\s*([0-9]+(?:[.,][0-9]+)?)\s*m²\s*$/i);if(_mN){var _pn=parseFloat(_mN[1]);var _wn=parseFloat(_mN[2].replace(",","."));var _rn=_wn/(1+_pn/100);var _rd=_rn.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});return "Metragem real "+_rd+"m² + "+_mN[1]+"% de perda = "+_wn.toLocaleString("pt-BR")+"m²";}var _mm=I.match(/^Metragem\s+real(?:\s+total)?\s+([0-9]+(?:[.,][0-9]+)?)\s*m²\s*$/i);if(!_mm)return I;var _vv=parseFloat(_mm[1].replace(",","."));if(!isFinite(_vv)||_vv<=0)return I;var _vd=_vv.toLocaleString("pt-BR",{minimumFractionDigits:2});var _vp=Math.round(_vv*(1+_ambPerdaPerc/100)).toLocaleString("pt-BR");return "Metragem real "+_vd+"m² + "+_ambPerdaPerc+"% de perda = "+_vp+"m²";});let A="";const _isPortaCat=C.toUpperCase()==="PORTA";if(u.length>1){A+=`<p class="desc-title">${u[0]}</p>`;A+=`<p class="desc-body">${u.slice(1).join('</p><p class="desc-body">')}</p>`;}else if(u.length===1){A+=`<p class="desc-title">${u[0]}</p>`;}else{A+=`<p class="desc-title">${d.descritivo}</p>`;}var _CORES=["Branco Diamante Essencial","Branco TX","Naturalle","Mont Blanc","Marrone","Cappuccino","Capuccino","Baby Grey","Grigio Nero","Light Brown","Milano","Smoked","Giz","Snow","Armani","Nevado","Italy Brown","Batman","Demolição","Demolicao","Customizado"];_CORES.sort(function(a,b){return b.length-a.length});var _corFromCat="";var _catStr=String(d.categoria||"").toUpperCase();for(var _ci=0;_ci<_CORES.length;_ci++){if(_catStr.indexOf(_CORES[_ci].toUpperCase())>=0){_corFromCat=_CORES[_ci];break;}}var _corShow=d.cor||_corFromCat;if(_corShow&&!_isRec&&!/^(INSUMOS|INSTALA)/i.test((d.descritivo||"").trim())){A+=`<p class="desc-body" style="color:#666">Acabamento / Cor: ${_corShow}</p>`;}if(C.toUpperCase()==="MARCENARIA"){const _accs=N.replace(/_/g," ").split(/\s*\|\s*/).map(function(s){return s.trim()}).filter(Boolean);if(_accs.length>=1){const _ext=_accs[0]||"";const _int=_accs[1]||"";let _accHtml=`<p class="desc-body" style="margin-top:6pt"><strong>Acabamentos:</strong></p>`;if(_ext)_accHtml+=`<p class="desc-body">• <strong>Externo:</strong> ${_ext}</p>`;if(_int)_accHtml+=`<p class="desc-body">• <strong>Interno:</strong> ${_int}</p>`;A+=_accHtml;}}if(C.toUpperCase().startsWith("RODAP")){const _amb=_ambByOrdem[d.ordem];const _mtlM=(d.descritivo||"").match(/^\s*([0-9]+[.,]?[0-9]*)\s*mtl/i);const _mtlV=_mtlM?_mtlM[1]:null;if(_amb&&_mtlV){A=`<p class="desc-title">${_amb}</p>`+`<p class="desc-body">metragem total ${_mtlV}mtl</p>`;}}const _itemLabel=U+"."+(++_rowIdx);g+=`<tr>
