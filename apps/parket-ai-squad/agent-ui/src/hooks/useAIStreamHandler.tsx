...
const useAIChatStreamHandler = () => {
  ...
  const handleStreamResponse = useCallback(
    async (input: string | FormData) => {
      ...
      // Auto-resume state: rastreia run_id + event_index pra retomar SSE quando rede cai.
      let runIdRef = ''
      let lastEventIdx = -1
      let runIsComplete = false
      const terminalEvents = new Set<string>([
        RunEvent.RunCompleted,
        RunEvent.TeamRunCompleted,
        RunEvent.RunError,
        RunEvent.TeamRunError,
        RunEvent.RunCancelled,
        RunEvent.TeamRunCancelled
      ])
      try {
        ...
        // background=true: run roda em asyncio.Task detached. Se conexão SSE cair,
        // run continua server-side e fica gravada no DB (status COMPLETED em vez de CANCELLED).
        ...
        const handleChunk = (chunk: RunResponse) => {
          // Auto-resume bookkeeping: rastreia run_id + event_index pra retry.
          if (typeof chunk.run_id === 'string' && !runIdRef) {
            runIdRef = chunk.run_id
          }
          const evtIdx = (chunk as unknown as { event_index?: number }).event_index
          if (typeof evtIdx === 'number' && evtIdx > lastEventIdx) {
            lastEventIdx = evtIdx
          }
          if (chunk.event && terminalEvents.has(chunk.event as string)) {
            runIsComplete = true
          }
          if (
        ...
        // Retry loop com auto-resume via /resume endpoint quando SSE cai mid-stream.
        ...
        while (attempts < MAX_ATTEMPTS) {
          ...
          if (isFirst) {
            ...
            const effectiveSessionId = String(newSessionId ?? sessionId ?? '')
            if (mode === 'team' && teamId) {
              apiUrl = `${endpointUrl}/teams/${teamId}/runs/${runIdRef}/resume`
            } else if (mode === 'agent' && agentId) {
              apiUrl = `${endpointUrl}/agents/${agentId}/runs/${runIdRef}/resume`
            } else {
              break
            }
            body = new FormData()
            body.append('last_event_index', String(lastEventIdx))
            body.append('session_id', effectiveSessionId)
          }
          ...
          if (runIsComplete) break
          // Stream encerrou sem evento terminal mas temos run_id → retry via resume
          if (runIdRef && lastError) {
            attempts++
