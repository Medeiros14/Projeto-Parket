...
} from "../components/journey-workbook-data";
import {
  ArrowLeft,
  ChevronDown,
  ChevronUp,
...
/* ─── Storage key ─── */
const STORAGE_KEY = "parket-journey-workbook";

/* ─── Tag badge colors ─── */
const TAG_COLORS: Record<string, { bg: string; color: string }> = {
  brutal: { bg: "rgba(220,80,60,0.15)", color: "#E87C6C" },
  "mata-ruído": { bg: "rgba(184,170,154,0.15)", color: ACCENT },
  decisiva: { bg: "rgba(220,80,60,0.15)", color: "#E87C6C" },
  ...
  "reputação": { bg: "rgba(100,180,120,0.15)", color: "#7BC48A" },
  sistema: { bg: "rgba(100,150,220,0.15)", color: "#7CA8D4" },
};

/* ═══════════════════════════════════════════════════════════ */
/* ─── Workbook State Types ─── */
...
export function JourneyWorkbookPage() {
  ...
  const updateFinal = (id: string, field: "rev" | "marc", value: string) => {
    setState((prev) => ({
      ...prev,
      finalAnswers: {
        ...prev.finalAnswers,
        [id]: { ...(prev.finalAnswers[id] || getDefaultFinal()), [field]: value },
      },
    }));
  };
  ...
  const isQuestionAnswered = (q: WorkbookQuestion): boolean => {
  ...
  const sectionProgress = (section: WorkbookSection): number => {
    const answered = section.questions.filter(isQuestionAnswered).length;
    return Math.round((answered / section.questions.length) * 100);
  };

  const totalAnswered = workbookSections.reduce(
    (sum, s) => sum + s.questions.filter(isQuestionAnswered).length,
    0
  );
  const finalAnswered = finalDeliveryItems.filter((fi) => {
    const a = state.finalAnswers[fi.id];
    return a && (a.rev.trim().length > 0 || a.marc.trim().length > 0);
  }).length;

  const totalProgress = Math.round(
    ((totalAnswered + finalAnswered) / (TOTAL_QUESTIONS + TOTAL_FINAL_ITEMS)) * 100
  );
  ...
  /* export as text */
  const exportText = () => {
    let text = "PARKET PISOS — JORNADA DO CLIENTE · WORKBOOK\n";
    text += "═".repeat(60) + "\n\n";
    ...
    text += `\n${"═".repeat(60)}\n`;
    text += `ENTREGA PRÁTICA — DIAGNÓSTICO EM DUAS COLUNAS\n`;
    text += `${"═".repeat(60)}\n\n`;

    for (const fi of finalDeliveryItems) {
    ...
    const stuckQuestions = workbookSections.flatMap((s) =>
      s.questions.filter((q) => state.answers[q.id]?.stuck).map((q) => `  #${q.id}: ${q.text}`)
    );
    text += stuckQuestions.length > 0 ? stuckQuestions.join("\n") : "  Nenhum identificado.";
    text += `\n\nProgresso: ${totalProgress}% (${totalAnswered + finalAnswered}/${TOTAL_QUESTIONS + TOTAL_FINAL_ITEMS})\n`;

    const blob = new Blob([text], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = "parket-jornada-workbook.txt";
    link.click();
  ...
  const renderQuestionCard = (q: WorkbookQuestion) => {
        ...
        <div className="mt-4">
          ...
          <div className="flex items-center justify-between mt-2">
            <span style={{ fontSize: "0.55rem", color: TEXT_DIM }}>
              {totalAnswered + finalAnswered}/{TOTAL_QUESTIONS + TOTAL_FINAL_ITEMS} respostas
            </span>
            {stuckCount > 0 && (
      ...
      <main ref={contentRef} className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto px-4 sm:px-8 py-8 sm:py-12 lg:py-16">
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <p className="mt-3" style={{ fontSize: "0.9rem", color: TEXT_MED, lineHeight: 1.7 }}>
                  80 perguntas + 7 diagnósticos para desenhar o processo operacional perfeito — de ponta a ponta.
                </p>
              </div>
              ...
              >
                ...
                <div className="space-y-4">
                  {[
                    { n: "1", text: "Responda em bullets, curto e objetivo.", accent: false },
                    {
                      n: "2",
                      text: "Para cada pergunta, responda 2 vezes: Revestimentos e Marcenaria.",
                      accent: true,
                    },
                    {
                      n: "3",
                    ...
                    {
                      n: "4",
                      text: "No final, você vai ter um desenho operacional pronto para implementar.",
                      accent: false,
                    },
                  ].map((rule) => (
              ...
              <div>
                ...
                >
                  14 Blocos + Entrega Prática
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {workbookSections.map((s) => (
                    <button
                      key={s.id}
          ...
          {workbookSections.map((section) => {
            ...
            const progress = sectionProgress(section);
            const currentIdx = workbookSections.findIndex((s) => s.id === section.id);
            const nextSection = workbookSections[currentIdx + 1];
            const prevSection = workbookSections[currentIdx - 1];
          ...
          {activeSection === "final" && (
            <div className="space-y-6">
              <div className="mb-8">
                ...
                <p className="mt-2" style={{ fontSize: "0.85rem", color: TEXT_MED, lineHeight: 1.6 }}>
                  O que eu preciso de você para desenhar corretamente sem ruído.
                  Responda em duas colunas: Revestimentos e Marcenaria.
                </p>
              </div>
              ...
              >
                <p style={{ fontSize: "0.8rem", lineHeight: 1.6, color: TEXT_MED }}>
                  <strong className="text-white">Me responda só isto</strong> — em duas colunas (Revestimentos e Marcenaria).
                  São 7 diagnósticos que, junto com as 80 perguntas acima, desenham o mapa completo da operação.
                </p>
              </div>

              {/* Final items */}
              {finalDeliveryItems.map((fi, idx) => {
                const answer = getFinal(fi.id);
                const answered = answer.rev.trim().length > 0 || answer.marc.trim().length > 0;
                return (
                  <div
                    key={fi.id}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <textarea
                          value={answer.rev}
                          onChange={(e) => updateFinal(fi.id, "rev", e.target.value)}
                          placeholder="Liste os 5 itens..."
                          rows={4}
                      ...
                      <div>
                        ...
                        <textarea
                          value={answer.marc}
                          onChange={(e) => updateFinal(fi.id, "marc", e.target.value)}
                          placeholder="Liste os 5 itens..."
                          rows={4}
                          className="w-full rounded-lg px-4 py-3 resize-y outline-none"
                          style={{
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <span style={{ fontSize: "0.65rem", color: TEXT_DIM }}>
                      de {TOTAL_FINAL_ITEMS} diagnósticos
                    </span>
                  </div>
                  <div>
              ...
              <div className="flex items-center pt-4">
                <button
                  onClick={() => setActiveSection(workbookSections[workbookSections.length - 1].id)}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-200"
                  style={{
                    fontSize: "0.75rem",
                    color: TEXT_MED,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          ...
          >
            ...
            <p style={{ fontSize: "0.8rem", color: TEXT_DIM, lineHeight: 1.6 }}>
              Todas as respostas e marcações de "travei aqui" serão apagadas. Essa ação não pode ser desfeita.
            </p>
