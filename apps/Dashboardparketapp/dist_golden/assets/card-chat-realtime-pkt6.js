...
(function () {
  ...
  async function pollNewMessages() {
    ...
    try {
      ...
      rows.forEach(function (m) {
        ...
        if (!cid) return;
        if (meEmail && m.user_email && String(m.user_email).toLowerCase() === meEmail) return;
  ...
  try {
    openGlobalChatRealtime(function (rec) {
      ...
      if (!cid) return;
      var meEmail = ckUserEmail();
      if (rec && rec.user_email && meEmail && String(rec.user_email).toLowerCase() === meEmail) return;
  ...
  function refreshInlineTabBadge() {
    ...
    bts.forEach(function (b) {
      ...
      if (idx === -1) return;
      // Se já tem badge, remove pra recalcular
      var existing = b.querySelector(".__pkt-tab-chat-badge");
      if (existing) existing.remove();
      // Tenta achar cardId via window.__pktCurrentOrcamentoCardId OU via mount point
      ...
      if (!cid) return;
      var info = unreadByContext[cid];
      if (!info || !info.count) return;
      var badge = document.createElement("span");
      badge.className = "__pkt-tab-chat-badge";
      badge.style.cssText = "display:inline-flex;align-items:center;justify-content:center;background:#EF4444;color:#fff;font-size:9px;font-weight:800;padding:1px 6px;border-radius:8px;margin-left:5px;line-height:1.3;box-shadow:0 1px 4px rgba(239,68,68,0.6);animation:__pktBadgePulse 2s ease-in-out infinite;";
