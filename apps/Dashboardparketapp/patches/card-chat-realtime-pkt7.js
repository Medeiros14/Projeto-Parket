...
(function () {
  ...
  function refreshKanbanBadges() {
    ...
    document.querySelectorAll(".__pkt-chat-unread").forEach(function (c) { c.classList.remove("__pkt-chat-unread"); });
    ...
    hits.forEach(function (textNode) {
      ...
      if (card.classList.contains("__pkt-chat-unread")) { seen.add(card); return; }
      card.classList.add("__pkt-chat-unread");
      card.title = "Nova mensagem no chat — clique no card pra abrir";
      seen.add(card);
    });
  }
  ...
  (function injectPulseCSS() {
    ...
    s.textContent = "@keyframes __pktBadgePulse{0%,100%{transform:scale(1);box-shadow:0 2px 8px rgba(239,68,68,0.6)}50%{transform:scale(1.12);box-shadow:0 2px 14px rgba(239,68,68,0.9)}}" +
      "@keyframes __pktCardUnreadPulse{0%,100%{box-shadow:inset 4px 0 0 #EF4444,0 0 0 1px rgba(239,68,68,0.35)}50%{box-shadow:inset 4px 0 0 #EF4444,0 0 0 3px rgba(239,68,68,0.55)}}" +
      ".__pkt-chat-unread{animation:__pktCardUnreadPulse 1.8s ease-in-out infinite;border-radius:inherit;}" +
      "button.__pkt-tab-chat-active{background:#EF4444 !important;color:#fff !important;animation:__pktCardUnreadPulse 1.8s ease-in-out infinite;}";
  ...
  try {
    openGlobalChatRealtime(function (rec) {
      ...
      if (!cid) return;
      var meEmail = ckUserEmail();
      var isSelf = rec && rec.user_email && meEmail && String(rec.user_email).toLowerCase() === meEmail;
      if (window.__pktChatOpenFor === cid) { markCardRead(cid); return; }
      var seen = loadLastSeen();
      var lastReadIso = seen[cid] || "1970-01-01T00:00:00Z";
      if (new Date(rec.created_at) <= new Date(lastReadIso)) return;
      var topicNorm = rec.topic ? String(rec.topic).toLowerCase().trim() : null;
      if (!unreadByContext[cid]) {
        if (!isSelf) showToast(rec, cid);
  ...
  // ── Botões inline (💬 Chat / 📎 Anexos) em cards — DESATIVADO a pedido do Will (2026-06-09) ──
  function decorateSimCards() { return; }
  // setInterval(decorateSimCards, 600);  // desativado
  // decorateSimCards();                    // desativado
  // Remove botões já injetados por instâncias antigas do script (cache)
  try { document.querySelectorAll(".pkt-card-actions").forEach(function(n){ n.remove(); }); } catch(_) {}
  ...
  function renderInlineChatPanel(mount) {
    ...
    // pkt6: chat inline aberto → marca como lido e flag pra realtime global pular badge
    try { markCardRead(contextId); window.__pktChatOpenFor = contextId; } catch(_) {}
    var _inlineCleanup = function () { if (window.__pktChatOpenFor === contextId) window.__pktChatOpenFor = null; };
    ...
    getTopicForCard(contextId).then(function (topicVal) {
      closeRT = openRealtime(contextId, topicVal, function (rec) {
        ...
        localMessages.sort(function (a, b) { return new Date(a.created_at) - new Date(b.created_at); });
        render();
        // chat aberto → mensagem nova é "lida na hora"
        try { markCardRead(contextId); } catch(_) {}
      });
    });
  ...
  // pkt6: badge no botão de aba "💬 Chat" dentro do modal do card. Mostra "💬 N"
  // ao lado do nome quando há msg não lida pra o cardId do modal aberto.
  // Move a aba "💬 Chat" pra logo depois da aba "WhatsApp"
  function repositionChatTab() {
    var bts = document.querySelectorAll('button');
    var chatBtn = null;
    for (var i = 0; i < bts.length; i++) {
      var t = (bts[i].textContent || "").trim();
      if (t.indexOf("💬 Chat") === 0 || t === "💬Chat") { chatBtn = bts[i]; break; }
    }
    if (!chatBtn) return;
    // Procura WhatsApp DENTRO do mesmo container (irmão direto)
    var parent = chatBtn.parentElement;
    if (!parent) return;
    var sibs = parent.children;
    var whatsBtn = null;
    for (var j = 0; j < sibs.length; j++) {
      if (sibs[j].tagName !== "BUTTON") continue;
      var txt = (sibs[j].textContent || "").trim();
      // "WhatsApp" exato OU começa com "WhatsApp " (sem * que é do form)
      if (txt === "WhatsApp") { whatsBtn = sibs[j]; break; }
    }
    if (!whatsBtn) return;
    // Já tá no lugar certo?
    if (chatBtn.previousElementSibling === whatsBtn) return;
    parent.insertBefore(chatBtn, whatsBtn.nextSibling);
    try { console.log("[pkt-chat] aba movida pra depois do WhatsApp"); } catch(_) {}
  }

  function refreshInlineTabBadge() {
    repositionChatTab();
    var bts = document.querySelectorAll('button');
    bts.forEach(function (b) {
      var tx = (b.textContent || "").trim();
      var idx = tx.indexOf("💬 Chat");
      if (idx === -1) {
        // Cleanup
        if (b.dataset.__pktChatRedOrigBg !== undefined) {
          b.style.background = b.dataset.__pktChatRedOrigBg;
          b.style.color = b.dataset.__pktChatRedOrigColor || "";
          delete b.dataset.__pktChatRedOrigBg;
          delete b.dataset.__pktChatRedOrigColor;
        }
        b.classList.remove("__pkt-tab-chat-active");
        var oldBadge = b.querySelector(".__pkt-tab-chat-badge");
        if (oldBadge) oldBadge.remove();
        return;
      }
      var mount = document.getElementById("pkt-modal-chat-mount");
      var cid = (mount && mount.getAttribute("data-card-id")) || window.__pktCurrentOrcamentoCardId || null;
      var info = cid ? unreadByContext[cid] : null;
      var hasUnread = info && info.count > 0;

      if (hasUnread) {
        if (b.dataset.__pktChatRedOrigBg === undefined) {
          b.dataset.__pktChatRedOrigBg = b.style.background || "";
          b.dataset.__pktChatRedOrigColor = b.style.color || "";
        }
        b.style.background = "#EF4444";
        b.style.color = "#fff";
        b.classList.add("__pkt-tab-chat-active");
      } else {
        if (b.dataset.__pktChatRedOrigBg !== undefined) {
          b.style.background = b.dataset.__pktChatRedOrigBg;
          b.style.color = b.dataset.__pktChatRedOrigColor || "";
        ...
        b.classList.remove("__pkt-tab-chat-active");
        var oldB = b.querySelector(".__pkt-tab-chat-badge");
        if (oldB) oldB.remove();
      }
    });
  }
  setInterval(refreshInlineTabBadge, 800);
