...
import { DEPARTMENTS, DEPT_MAP } from "./data";
...
function JornadaCliente({ T, onClose }: { T: ThemeTokens; onClose: () => void }) {
  const clientes = buildJornada();
...
export function NavonaDashboard({ onSelectDept, onGoInstaParket, onBack, theme, onToggleTheme, T }: NavonaDashboardProps) {
  const [hovered, setHovered] = useState<string | null>(null);
  const [showJornada, setShowJornada] = useState(false);
  const [search, setSearch] = useState("");

  const totalCards = DEPARTMENTS.reduce((acc, d) => acc + d.columns.reduce((a, c) => a + c.cards.length, 0), 0);
  ...
  return (
    <div style={{ backgroundColor: T.bg, minHeight: "100vh", color: T.textPrimary, transition: "background 0.35s, color 0.35s" }}>
      ...
      <div style={{ paddingTop: 64 }}>
        ...
        {/* Jornada */}
        {showJornada && <JornadaCliente T={T} onClose={() => setShowJornada(false)} />}
