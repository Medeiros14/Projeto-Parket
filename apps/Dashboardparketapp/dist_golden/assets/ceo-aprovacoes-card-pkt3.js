...
(function () {
  ...
  async function fetchChatCounts(cardIds) {
    ...
    const out = {};
    try {
      // 1 request per cardId — Supabase REST doesn't easily group by, but count via head request is fast
  ...
  async function abrirOverlay() {
    ...
    chatCountsCache = await fetchChatCounts(ids);
    renderLista();
  }
  ...
  async function postarComentario(cardId, texto, tipo, item) {
    ...
    const body = {
      ...
      topic: item && item.cliente ? item.cliente : null,
      user_name: "🟡 CEO — " + nome,
      user_email: email,
      avatar: "👑",
      msg: prefix + "\n" + texto,
  ...
  async function acaoDecisao(act, cardId, btn, motivoOverride) {
    ...
    try {
      ...
      // Notifica o grupo Parket — Orçamentos no WhatsApp
      try {
