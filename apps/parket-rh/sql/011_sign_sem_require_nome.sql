-- 011 — documento_public_sign: nome/cpf NÃO são mais obrigatórios no payload
-- (o admin pré-preenche em nome_informado/cpf_informado; o signatário só faz selfie+doc+assinatura)
...
BEGIN
  ...
  UPDATE rh.documentos_emitidos SET
    ...
    status = 'assinado',
    assinado_em = v_now,
    -- Só sobrescreve quando o payload realmente trouxe (raro; normalmente vem pré-preenchido pelo admin)
    nome_informado = CASE WHEN v_nome IS NOT NULL AND v_nome <> '' THEN v_nome ELSE nome_informado END,
    cpf_informado = CASE WHEN v_cpf <> '' THEN v_cpf ELSE cpf_informado END,
    updated_at = v_now
  WHERE id = v_id;
