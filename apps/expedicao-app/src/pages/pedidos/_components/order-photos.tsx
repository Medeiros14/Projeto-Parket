...
import { Camera, Upload, Trash2, ZoomIn, PackageCheck, Navigation, ImageIcon, X } from "lucide-react";
...
const photoTypeConfig = {
  separacao: { label: "Separação", icon: PackageCheck, color: "bg-amber-500/15 text-amber-400" },
  entrega: { label: "Entrega", icon: Navigation, color: "bg-emerald-500/15 text-emerald-400" },
  outros: { label: "Outros", icon: ImageIcon, color: "bg-muted text-muted-foreground" },
};

type PhotoType = "separacao" | "entrega" | "outros";
...
export function OrderPhotos({ orderId, orderStatus }: OrderPhotosProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const cameraInputRef = useRef<HTMLInputElement>(null);

  const generateUploadUrl = useMutation(api.orderPhotos.generateUploadUrl);
  const addPhoto = useMutation(api.orderPhotos.addOrderPhoto);
  const deletePhoto = useMutation(api.orderPhotos.deleteOrderPhoto);
  const photos = useQuery(api.orderPhotos.getOrderPhotos, { orderId });

  const [uploading, setUploading] = useState(false);
  const [photoType, setPhotoType] = useState<PhotoType>(() => {
    if (orderStatus === "em_separacao") return "separacao";
    if (orderStatus === "em_rota" || orderStatus === "entregue") return "entrega";
    return "outros";
  });
  ...
  const handleDelete = async (id: Id<"orderPhotos">) => {
    try {
  ...
  const grouped = photos?.reduce<Record<PhotoType, typeof photos>>((acc, p) => {
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              placeholder="Ex: Caixas separadas..."
              value={caption}
        ...
        Object.entries(photoTypeConfig).map(([type, cfg]) => {
          ...
          return (
            <div key={type} className="space-y-2">
              <div className={`flex items-center gap-2 text-xs font-semibold px-2 py-1 rounded-full w-fit ${cfg.color}`}>
                <Icon className="w-3 h-3" />
                {cfg.label} ({typePhotos.length})
              </div>
              <div className="grid grid-cols-3 gap-2">
                {typePhotos.map((photo) => (
                  <div key={photo._id} className="relative group aspect-square rounded-lg overflow-hidden bg-muted border">
                    {photo.url ? (
                      <img
                        src={photo.url}
              ...
              <p className="text-[10px] text-muted-foreground px-1">
                Por {typePhotos[0]?.userName} · {format(new Date(typePhotos[0]?._creationTime), "dd/MM/yy HH:mm", { locale: ptBR })}
              </p>
            </div>
          );
        })
