...
- add_item          {ambiente, categoria, especie?, dimensao?, m2, preco_unitario?, descritivo?, modo?: "material", dimensao_exclusiva?}
- reordenar_ambiente {ambiente|id, direcao: "cima"|"baixo"}  (move 1 posição — chame N vezes pra mover N posições)
- reordenar_item    {id, direcao: "cima"|"baixo"}            (só dentro do mesmo ambiente; entre ambientes use mover_item)
- ajustar_preco_categoria {categoria, aumento_pct|desconto_pct|fator}  (aplica multiplicador no preco_unitario de TODOS os itens dessa cat — não mexe em desconto_perc/valor da sim)
...
from app.agents.base import db, make_model
...
**FECHAR EM VALOR ALVO (usuário pede "ajusta os painéis pra fechar em R$X")**: O DIVISOR é a soma das **METRAGENS COBRADAS** (com perda), NÃO das metragens informadas. Você vê a metragem cobrada no Contexto atual como `metragem_cobrada` ou pode calcular: `Σ ceil(ceil(m_inf) × (1 + perda/100))` de todos os itens da categoria. Fórmula: `novo_preco = valor_alvo / Σ metragens_cobradas`. Depois `update_itens_bulk {"campo":"preco_unitario","valor":novo_preco,"categoria":"painel"}` — o sistema recalcula valor_total corretamente com perda. Se você dividir pela metragem informada (sem perda), o total final não bate com o alvo (fica ~10% maior).
...
def build_valoria_assistant() -> Agent:
    from app.tools import catalogo as catalogo_tools
    from app.tools import teca_nucleo
    return Agent(
        id="valoria_assistant",
        name="valoria_assistant",
        # 21/07/2026: Will pediu volta pro opus-4-7. max_tokens=32k (teto do
        # opus) — fable-5 aceitava 128k, opus não. timeout=3600s continua pra
        # run não-streamed de anexo.
        model=make_model(model_id="claude-opus-4-7", max_tokens=32000, timeout=3600.0),
        db=db(),
        tools=[
            handoff.log_atividade,
            catalogo_tools.catalogo_consultar,
            catalogo_tools.catalogo_editar_preco,
            teca_nucleo.teca_aprendizados_listar,
            teca_nucleo.teca_aprendizado_criar,
        ],
