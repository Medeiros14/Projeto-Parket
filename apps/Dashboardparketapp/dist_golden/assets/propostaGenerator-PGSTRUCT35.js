...
const _rods=P.filter(d=>(d.descritivo||"").toUpperCase().startsWith("RODAPÉ"));for(const _rd of _rods){const _rl=(_rd.descritivo||"").split("\n").filter(l=>l.trim()&&!/^RODAPÉ$/i.test(l.trim()));if(_rl.length>0)o.push(_rl.join(" "))}if(o.length>_lenBefore){var _nCaps=String(n||"").toUpperCase().trim();/*PKT-CAPS-ALL-20260720: Will 20/07 — obs auto de QUALQUER categoria em CAPS.*/{for(let _kk=_lenBefore;_kk<o.length;_kk++)o[_kk]=String(o[_kk]||"").toUpperCase();}_obsAuto[t]=o.slice(_lenBefore).join("\n");}}w=o.join(`
...
INSUMOS DE`)||t.match(/^INSUMOS\b/)?1:t.startsWith("INSTALAÇÃO")||t.includes("INSTALAÇÃO E GEST")?2:0};const _sortedCatKeys=Object.keys(x).sort((a,b)=>{const au=a.split("||")[0].toUpperCase(),bu=b.split("||")[0].toUpperCase();const aIsMat=au==="MATERIAL"?2:0,bIsMat=bu==="MATERIAL"?2:0,aIsRod=0,bIsRod=0,aTail=aIsMat+aIsRod,bTail=bIsMat+bIsRod;if(aTail!==bTail)return aTail-bTail;const _catOrder=["PISO","RODAPÉ","RODAPE","FORRO","PAINEL","DECK","PORTA","ESCADA","LOGISTICA","LOGÍSTICA","COMPRAS","INSUMOS"];const _ai=_catOrder.indexOf(au),_bi=_catOrder.indexOf(bu);if(au===bu)return 0;if(_ai===-1&&_bi===-1)return au.localeCompare(bu);if(_ai===-1)return 1;if(_bi===-1)return-1;return _ai-_bi});const _ambByOrdem={};for(const _cat of Object.keys(x)){if(_cat.toUpperCase().startsWith("RODAP"))continue;for(const _it of x[_cat]){const _l=(_it.descritivo||"").toUpperCase();if(_l.includes("INSUMOS")||_l.includes("INSTALAÇÃO"))continue;const _fl=(_it.descritivo||"").split("\n")[0].trim();if(_fl&&!_ambByOrdem[_it.ordem])_ambByOrdem[_it.ordem]=_fl;}}let _allObsPorProduto=[];let g_main="",g_extras="",_totMain=0,_totExtras=0,_gBefore=0,_catsMain=[],_catsExtras=[];const _isExtraCat=c=>(String(e.id||"").toLowerCase()==="aaaa1111-2222-4333-8444-202608060001"?/^(PAINEL|PORTA|MARCEN|ESCADA|SAUNA|REVEST|ADEGA)/i:/^(PAINEL|PORTA|MARCEN)/i).test(String(c||""))/*PKT-SPLIT3-20260806*/;const _catDisplayName=c=>{const _n=String(c||"").split("||")[0].trim();const _map={"RODAPE":"Rodapé","RODAPÉ":"Rodapé"};return _map[_n.toUpperCase()]||_n.charAt(0).toUpperCase()+_n.slice(1).toLowerCase();};const _splitCat=(_o,_n)=>{var _pktSplitAllow=["6ed5b63b-0a96-4002-b529-9eba4248018f","aaaa1111-2222-4333-8444-202608060001"];if(_pktSplitAllow.indexOf(String(e.id||"").toLowerCase())<0)return;const _new=g.slice(_gBefore);g=g.slice(0,_gBefore);const _dn=_catDisplayName(_o);if(_isExtraCat(_o)){g_extras+=_new;_totExtras+=_n;if(!_catsExtras.includes(_dn))_catsExtras.push(_dn);}else{g_main+=_new;_totMain+=_n;if(!_catsMain.includes(_dn))_catsMain.push(_dn);}};for(const o of _sortedCatKeys){_gBefore=g.length;const _agg=(()=>{const _arr=x[o];const _map=new Map();const _out=[];const _isMarc=o.toUpperCase().startsWith("MARCENARIA");let _marcExtraTotal=0;for(const _it of _arr){const _d=(_it.descritivo||"").toUpperCase().trim();/*PKT-LAYOUT-REFPDF-20260715: aux INSUMOS/INSTALAÇÃO agregados em 1 linha por categoria com label limpa. Modelo PDF referência.*/const _key=(_d.startsWith("INSUMOS")?"_INSUMOS":(_d.startsWith("INSTALA")?"_INSTALACAO":null));if(_key){if(_isMarc){_marcExtraTotal+=Number(_it.valor||0);continue;}if(_map.has(_key)){const _ex=_map.get(_key);_ex.valor=Number(_ex.valor||0)+Number(_it.valor||0);}else{var _cleanDesc=(_key==="_INSUMOS")?"INSUMOS":"INSTALAÇÃO E GESTÃO DE OBRAS";const _copy={..._it,descritivo:_cleanDesc};_map.set(_key,_copy);_out.push(_copy)}}else{_out.push(_it)}}if(_isMarc&&_marcExtraTotal>0&&_out.length>0){const _baseTotal=_out.reduce((s,it)=>s+Number(it.valor||0),0);if(_baseTotal>0){_out.forEach(it=>{const _share=Number(it.valor||0)/_baseTotal;it.valor=Number(it.valor||0)+_marcExtraTotal*_share;});}else{_out[0].valor=Number(_out[0].valor||0)+_marcExtraTotal;}}return _out;})();const t=[..._agg].sort((d,l)=>{var _isPC=String((d.categoria||"")).toUpperCase().indexOf("PORTA")>=0&&String((l.categoria||"")).toUpperCase().indexOf("PORTA")>=0;if(_isPC&&!((d.meta||{}).ordem_manual||(l.meta||{}).ordem_manual)){var _md=String((d.categoria||"")).split("||")[1]||"";var _ml=String((l.categoria||"")).split("||")[1]||"";if(_md!==_ml)return _md.localeCompare(_ml);}const u=$(d.descritivo),A=$(l.descritivo);if(u!==A)return u-A;return d.ordem-l.ordem;}),n=t.reduce((d,l)=>d+l.valor,0),__cParts=o.split("||"),v=__cParts[0],P=__cParts[1]||"",C=v;let N=P||"";var _cor=(t.find(d=>d.cor)||{}).cor||"";if(_cor){var _corUp=String(_cor).toUpperCase().trim();if(_corUp&&!N.toUpperCase().includes(_corUp)){N=(N.trim()+" "+_corUp).trim();}}if(C.toUpperCase()!=="DECK"){N=N.replace(/^\s*DECK\s+/i,"").trim();}if(C.toUpperCase()!=="SAUNA"){N=N.replace(/^\s*SAUNA\s+/i,"").trim();}if(C.toUpperCase()==="RODAPÉ"||C.toUpperCase()==="RODAPE"){const _firstRod=t.find(d=>!(d.descritivo||"").toUpperCase().startsWith("INSUMOS")&&!(d.descritivo||"").toUpperCase().startsWith("INSTALAÇÃO"));if(_firstRod){const _rl=(_firstRod.descritivo||"").split("\n").filter(l=>l.trim()&&!/^RODAPÉ$/i.test(l.trim()));if(_rl.length>0){const _nm=_rl[0].replace(/^[\d.,]+\s*mtl?\s*/i,"").replace(/\s*RODAP[ÉE]\s*/i,"").replace(/\s*—.*$/,"").replace(/\s*20X20XCOMP\.VARIAVEL\s*/i,"").trim();if(_nm&&!P)N=_nm.toUpperCase()}}};if(!N)for(const d of t){const l=(d.descritivo||"").split(`
`).find(z);if(l){N=B(l);break}}if((C.toUpperCase()==="PAINEL"||C.toUpperCase()==="REVESTIMENTO")&&/LAMIN/i.test(N)){N=N.replace(/\s*\d+\/\d+\s*[×x]\s*\d+\s*MM?/gi,"").replace(/\s+/g," ").trim();}if(C.toUpperCase()!=="PORTA"&&!/^MAO\s+DE\s+OBRA/i.test(C)){g+=`<tr>
      <td class="cat-c">${/BRISE/i.test(String(t&&t.length?t[0].categoria:"")) ? "BRISE" : C.toUpperCase()} </td>
      <td class="cat-l">${(function(_x){var _w=/^PAINEL$/i.test(C);var _r=(__cParts[2]||"").trim();/*PKT-FORMATO-DIM-PREFIX-20260730: Will 30/07 — dim "Chevron · 15/3..." / "Espinha de Peixe · 1/6..." (formato-tab da Valoria) strippa o prefixo E appenda ao _x (nome). Guard anti-dup (regex \\b) evita "CHEVRON CHEVRON".*/var _fpM=_r.match(/^(R[éeê]gua(?:\s+\d+mm)?|Maci[çc]o|Espinha\s+de\s+Peixe|Chevron|Espessura[^·]*)\s+·\s+/i);if(_fpM){_r=_r.replace(/^[^·]+·\s+/,"").trim();var _fpUp=_fpM[1].toUpperCase().replace(/\s+/g," ");if(/^(CHEVRON|ESPINHA DE PEIXE)$/.test(_fpUp)&&!new RegExp("\\b"+_fpUp.replace(/\s+/g,"\\s+")+"\\b","i").test(_x)){_x=(_x+" "+_fpUp).trim();}}if(!_r||!/\d.{0,12}[×xX].{0,12}\d/.test(_r)){var _n=(__cParts[1]||"").toUpperCase();if(/^(FORRO|PAINEL|REVESTIMENTO)$/i.test(C)&&(/\bRIPADO\b/.test(_n)||/\bTOBLERONE\b/.test(_n))){_r=/^FORRO$/i.test(C)?"1,8 × 3,5cm × comp. variável":"1,8 × 5cm × comp. variável";}}var _isLam=_r && /\d+\/\d+\s*[×x]/i.test(_r);var _hasOtherAcab=/^\s*(RIPADO|TOBLERONE|MUXARABI|ASSOALHO|MACI[ÇC]O|MOLDURA|LACA|BLINDADA|REGUA|RÉGUA)\b/i.test(_x);var _name=_x;_name=_name.replace(/\s+MACI[ÇC][OA]\s*$/i,"").trim();if(/^(PISO|PAINEL|REVESTIMENTO)$/i.test(C)&&_r&&/^\s*1\/6\s*[×xX]/.test(_r)&&!/ESPINHA\s*DE\s*PEIXE|CHEVRON/i.test(_name)){_name=_name.replace(/^ESPINHA\s+(?!DE\s+PEIXE)/i,"").trim();_name=(_name.trim()+" ESPINHA DE PEIXE").trim();}var _hasDimPattern=_r&&/\d.{0,12}[×xX].{0,12}\d/.test(_r);var _suppressDim=/\b(TOBLERONE|MUXARABI)\b/i.test((__cParts[1]||""));var _showDim=_hasDimPattern&&!_suppressDim;return _showDim?(_name+'<br><span class="cat-l-dim">'+_mmToCm(_r)+'</span>'):_name;})((C.toUpperCase()==="PORTA"?N.replace(/\b(?:DN150|RO82TOP|CIR|GERIS|ITALY\s+LINE|3D)\b/gi," ").replace(/\b(ASSOALHO|LACA|LAMINA|MOLDURA[_ ]VIDRO|MUXARABI|RIPADO|TOBLERONE)\b\s+(?=\S)/gi," ").replace(/\s+/g," ").trim().split(" ").filter(Boolean).filter((_v,_i,_a)=>_i===0||_v.toUpperCase()!==_a[_i-1].toUpperCase()).join(" ").replace(/_/g," "):((function(){var _t=N.replace(/_/g," "),_c=C.toUpperCase();if(_c!=="FORRO"&&_c!=="PAINEL"&&_c!=="REVESTIMENTO"&&_c!=="MARCENARIA")_t=_t.replace(/^\s*(ASSOALHO|L[AÂ]MINA|RIPADO|TOBLERONE|MUXARABI|MOLDURA[_ ]VIDRO|LACA|R[ÉE]GUAS?)\s+/i,"");if(_c==="MARCENARIA"){return _t.split(/\s*\|\s*/).map(function(s){return s.trim().toUpperCase()}).filter(Boolean).join(" / ");}var _isLam=(_c==="PAINEL"||_c==="REVESTIMENTO")&&_t&&!/LAMINA|RIPADO|TOBLERONE|DECK|MACI[ÇC]O|MOLDURA|MUXARABI|DEMOLI[ÇC][AÃ]O|MULTIBLOC|LACA|ASSOALHO|R[ÉE]GUA/i.test(_t);return _t.replace(/\s+/g," ").trim().split(" ").filter(Boolean).filter((_v,_i,_a)=>_i===0||_v.toUpperCase()!==_a[_i-1].toUpperCase()).join(" ")})())))}</td>
      <td class="cat-v"></td>
    </tr>`;}let _recIdx=0,_ambSum=0,_ambCount=0,_subtotalAdded=false,_rowIdx=0,_ambMetragemSum=0,_ambCPSum=0,_ambPerdaPerc=10,_lastPortaModel="";t.forEach((d,l)=>{d.descritivo=(d.descritivo||"").replace(/Metragem\s+real\s+total\s*:/gi,"Metragem real").replace(/Metragem\s+real\s+total\s/gi,"Metragem real ").replace(/\\n/g,"\n").replace(/(\S)\s+(Metragem\s+real)/gi,"$1\n$2");const _hdU=((d.descritivo||"").split("\n")[0]||"").trim().toUpperCase();const _isAmbItem=!/^(INSTALAÇÃO\s+(E|DE)\s+GEST|INSTALACAO\s+(E|DE)\s+GEST|INSUMOS\b|RECORTES\b)/.test(_hdU);if(_isAmbItem){_ambSum+=(+d.valor||0);_ambCount++;var _amDesc=d.descritivo||"";var _amLns=_amDesc.split("\n");for(var _li=0;_li<_amLns.length;_li++){var _amPP=_amLns[_li].match(/Metragem\s+real(?:\s+total)?\s+[0-9]+(?:[.,][0-9]+)?\s*m²\s*\+\s*([0-9]+)\s*%\s*de\s+perda/i);if(_amPP){_ambPerdaPerc=parseFloat(_amPP[1])||_ambPerdaPerc;}var _amM=_amLns[_li].match(/^[\s​]*Metragem\s+real(?:\s+total)?\s+([0-9.,]+)\s*m²/i);/*PKT-MTOT-ZWSP-20260806*/if(_amM){_ambMetragemSum+=Math.ceil(parseFloat(_amM[1].replace(/\./g,"").replace(",",".")));var _amCP=_amLns[_li].match(/=\s*([0-9.,]+)\s*m²/i);if(_amCP){_ambCPSum+=Math.ceil(parseFloat(_amCP[1].replace(/\./g,"").replace(",",".")));}break;}/*PKT-MTOT-REGEX*//*PKT-MTOT-FIX*/_amM=_amLns[_li].match(/^[\s​]*Metragem\s+total\s+com\s+perda\s+de\s+([0-9]+)\s*%\s*([0-9]+(?:[.,][0-9]+)?)\s*m²/i);if(_amM){_ambMetragemSum+=parseFloat(_amM[2].replace(/\./g,"").replace(",","."))/(1+parseFloat(_amM[1])/100);/*PKT-MTOT-FIX2*/_ambPerdaPerc=parseFloat(_amM[1])||_ambPerdaPerc;_ambCPSum+=Math.ceil(parseFloat(_amM[2].replace(/\./g,"").replace(",",".")));break;}}}else if(!_subtotalAdded&&_ambCount>=2){g+=`<tr><td class="item-c"></td><td class="item-l">Metragem real total ${Math.ceil(_ambMetragemSum)}m² + ${_ambPerdaPerc}% de perda = ${_ambCPSum>0?_ambCPSum:Math.ceil(Math.round(Math.ceil(_ambMetragemSum)*(1+_ambPerdaPerc/100)*1e9)/1e9)}m²</td><td class="item-v">${E(_ambSum)}</td></tr>`;_subtotalAdded=true;}const _isRec=(d.descritivo||"").toUpperCase().startsWith("RECORTES");const u=(d.descritivo||"").split(`
`).map(I=>I.trim()).filter(Boolean).filter(I=>!z(I)&&!/^__RECDATA__:/.test(I)&&!/^\s*\d+\s+unidade(s)?\s*$/i.test(I)&&!(/^\s*\d+\/\d+\s*[×x]\s*\d+\s*(MM)?\s*$/i.test(I))&&!(C.toUpperCase()==='MARCENARIA'&&(/^\s*\(inclui ferragens/i.test(I)||/^\s*Madeiras:/i.test(I)||/^\s*Metragem\s+real\b/i.test(I)))&&!(/^(R[éê]gua|Maci[çc]o|Espinha de Peixe|Chevron|L[âa]mina|Ripado|Toblerone|Muxarabi|Assoalho|Espessura\s+\d)[^a-z]{0,20}$/i.test(I))) /*PKT-NOSUBG-STRIP*/.map(I=>I.replace(/[a-záéíóúç]+_[a-záéíóúç_]+/gi,W=>W.replace(/_/g," "))).map(function(I){var _hd=((d.descritivo||"").split("\n")[0]||"").trim().toUpperCase();if(/^INSUMOS|^RECORTES/.test(_hd)){var _insM=I.match(/^(\s*INSUMOS\s+DE\s+[A-ZÁÀÂÃÉÊÍÓÔÕÚÇ]+)\b/i);if(_insM)return _insM[1].toUpperCase();return I;}if(/^INSTALA(?:ÇÃO|CAO)\s+(E|DE)\s+GEST/i.test(_hd)){if(/^Metragem/i.test(I) && _ambMetragemSum>0){return "Metragem total "+Math.round(_ambMetragemSum)+"m²";}return I;}var __pktBRNum=function(s){s=String(s).trim();if(s.indexOf(".")>-1&&s.indexOf(",")>-1)s=s.replace(/\./g,"").replace(",",".");else if(s.indexOf(",")>-1)s=s.replace(",",".");else if(/^\d{1,3}(\.\d{3})+$/.test(s))s=s.replace(/\./g,"");return parseFloat(s)};var _mmF=I.match(/^Metragem\s+real(?:\s+total)?\s+([0-9]+(?:[.,][0-9]+)?)\s*m²\s*\+\s*([0-9]+)\s*%\s*de\s+perda\s*=\s*([0-9]+(?:[.,][0-9]+)?)\s*m²/i);if(_mmF){var _v1=__pktBRNum(_mmF[1]);var _vP=parseFloat(_mmF[2]);return "Metragem real "+Math.ceil(_v1).toLocaleString("pt-BR")+"m² + "+_mmF[2]+"% de perda = "+Math.ceil(Math.round(Math.ceil(_v1)*(1+_vP/100)*1e9)/1e9).toLocaleString("pt-BR")+"m²";}if(/^Metragem\s+real\s+[0-9.,]+\s*m²\s*\+\s*[0-9]+\s*%\s*de\s+perda/i.test(I))return I;var _mN=I.match(/^Metragem\s+total\s+com\s+perda\s+de\s+([0-9]+)\s*%\s*([0-9]+(?:[.,][0-9]+)?)\s*m²\s*$/i);if(_mN){var _pn=parseFloat(_mN[1]);var _wn=__pktBRNum(_mN[2]);var _rn=_wn/(1+_pn/100);var _rd=_rn.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});return "Metragem real "+Math.ceil(_rn).toLocaleString("pt-BR")+"m² + "+_mN[1]+"% de perda = "+Math.ceil(Math.round(Math.ceil(_rn)*(1+_pn/100)*1e9)/1e9).toLocaleString("pt-BR")+"m²";}var _mm=I.match(/^Metragem\s+real(?:\s+total)?\s+([0-9]+(?:[.,][0-9]+)?)\s*m²\s*$/i);if(!_mm)return I;/*PKT-NOPERDA-BARE-20260723: linha bare "Metragem real Xm²" (sem "+perda") = item com perda_pct=0 (Will 23/07 "tira a perda"). Antes forçava _ambPerdaPerc default e revertia o pedido. Agora passa direto.*/return I;}).map(function(I){/*PKT-CAPS-ALL-20260720: Will 20/07 — toda categoria em CAPS.*/return String(I||"").toUpperCase();});let A="";const _isPortaCat=C.toUpperCase()==="PORTA";if(_isRec){const _recItems=u.slice(1).flatMap(function(_ln){return String(_ln||"").replace(/\.$/,"").split(/\s+e\s+|\s*\|\s*/).map(function(_s){return _s.trim()}).filter(Boolean)});A+=`<p class="desc-title">Detalhamento de Projeto</p>`;if(_recItems.length>0)A+=`<p class="desc-body">${_recItems.join('</p><p class="desc-body">')}</p>`;}else if(_isPortaCat){var _portaLns=u.filter(function(_l){return !/^\s*\d+\s+Porta(s?)\s*$/i.test(_l);});if(_portaLns.length>=2){A+=`<p class="desc-title">${_portaLns[0]}</p>`;A+=`<p class="desc-body">${_portaLns.slice(1).join('</p><p class="desc-body">')}</p>`;}else if(_portaLns.length===1){A+=`<p class="desc-title">${_portaLns[0]}</p>`;}else{A+=`<p class="desc-title">${u[0]||"PORTA"}</p>`;}}else if(u.length>1){A+=`<p class="desc-title">${u[0]}</p>`;A+=`<p class="desc-body">${u.slice(1).join('</p><p class="desc-body">')}</p>`;}else if(u.length===1){A+=`<p class="desc-title">${u[0]}</p>`;}else{A+=`<p class="desc-title">${d.descritivo}</p>`;}A=A.replace(/<p class="desc-title">(\s*Metragem)/gi,'<p class="desc-body">$1');A=A.replace(/<p class="desc-title">(\u200B)/g,'<p class="desc-body">$1');/*PKT-DESC-FIXO-NOBOLD-20260804: linha ZWSP (descritivo_fixo) nunca vira titulo/negrito (Will 04/08)*/var _CORES=["Branco Diamante Essencial","Branco TX","Naturalle","Mont Blanc","Marrone","Cappuccino","Capuccino","Baby Grey","Grigio Nero","Light Brown","Milano","Smoked","Giz","Snow","Armani","Nevado","Italy Brown","Batman","Customizado"];_CORES.sort(function(a,b){return b.length-a.length});var _corFromCat="";var _catStr=String(d.categoria||"").toUpperCase();for(var _ci=0;_ci<_CORES.length;_ci++){if(_catStr.indexOf(_CORES[_ci].toUpperCase())>=0){_corFromCat=_CORES[_ci];break;}}var _corShow=d.cor;if(_corShow&&!_isRec&&!/^(INSUMOS|INSTALA)/i.test((d.descritivo||"").trim())){A+=``;}if(C.toUpperCase()==="MARCENARIA"&&String(d.descritivo||"").indexOf("\u200B")===-1){/*PKT-DESC-FIXO-MARC-20260804*/const _accs=N.replace(/_/g," ").split(/\s*\|\s*/).map(function(s){return s.trim()}).filter(Boolean);if(_accs.length>=1){const _ext=_accs[0]||"";const _int=_accs[1]||"";let _accHtml=`<p class="desc-body" style="margin-top:6pt"><strong>Acabamentos:</strong></p>`;if(_ext)_accHtml+=`<p class="desc-body">• <strong>Externo:</strong> ${_ext}</p>`;if(_int)_accHtml+=`<p class="desc-body">• <strong>Interno:</strong> ${_int}</p>`;A+=_accHtml;}}if(C.toUpperCase().startsWith("RODAP")){const _amb=_ambByOrdem[d.ordem];const _mtlM=(d.descritivo||"").match(/^\s*([0-9]+[.,]?[0-9]*)\s*mtl/i);const _mtlV=_mtlM?_mtlM[1]:null;if(_amb&&_mtlV){A=`<p class="desc-body">${_amb}</p>`+`<p class="desc-body">metragem total ${_mtlV}mtl</p>`;}else{A=A.replace(/desc-title/g,"desc-body");}}if(C.toUpperCase()==="PORTA"){var _pmCur=(String(d.categoria||"").split("||")[1]||"").trim().split(/(<[^>]+>)/).map(function(_s,_i){return _i%2===0?_s.toUpperCase():_s;}).join("");if(_pmCur&&_pmCur!==_lastPortaModel){_lastPortaModel=_pmCur;g+=`<tr><td class="cat-c">PORTA </td><td class="cat-l">${_pmCur}</td><td class="cat-v"></td></tr>`;}}const _itemLabel=U+"."+(++_rowIdx);g+=`<tr>
        <td class="item-c">${_itemLabel}</td>
        <td class="item-l">${A}</td>
        <td class="item-v">${E(d.valor)}</td>
      </tr>`}),(function(){if(!_subtotalAdded&&_ambCount>=2&&(_ambMetragemSum>0||_ambCPSum>0)){g+=`<tr><td class="item-c"></td><td class="item-l">Metragem real total ${Math.ceil(_ambMetragemSum)}m² + ${_ambPerdaPerc}% de perda = ${_ambCPSum>0?_ambCPSum:Math.ceil(Math.round(Math.ceil(_ambMetragemSum)*(1+_ambPerdaPerc/100)*1e9)/1e9)}m²</td><td class="item-v">${E(_ambSum)}</td></tr>`;_subtotalAdded=true;}})(),g+=`<tr>
      <td class="sub-c"></td>
      <td class="sub-l">Valor parcial ${(function(_c){var _u=String(_c||"").toUpperCase().trim();if(/M[ÃA]O\s+DE\s+OBRA/.test(_u))return "da MÃO DE OBRA";var _CATS=["MARCENARIA","REVESTIMENTO","ADEGA","RODAPÉ","RODAPE","SAUNA","PAINEL","ESCADA","PORTA","FORRO","DECK","PISO"];var _FEM={"PORTA":1,"SAUNA":1,"ESCADA":1,"MARCENARIA":1,"ADEGA":1};for(var _i=0;_i<_CATS.length;_i++){if(_u.indexOf(_CATS[_i])>=0){var _c2=_CATS[_i].replace("RODAPE","RODAPÉ");return (_FEM[_c2]?"da":"do")+" "+_c2;}}var _w=(_u.split(/\s+/)[0]||_u);return "do "+_w;})(C)} </td>
      <td class="sub-v">${E(n)}</td>
    </tr>`,U++,_splitCat(o,n)}(y+_freteVal)>0&&(g+=`<tr>
    ...
    <td class="cond-v"></td>
  </tr>`;let h="";if(e.composicao_faturamento||e.anexos){h+=`
  ...
  </div><!-- end extra page -->`}const _pktSplitAllow=["6ed5b63b-0a96-4002-b529-9eba4248018f","aaaa1111-2222-4333-8444-202608060001"];const _pktSplit=_pktSplitAllow.indexOf(String(e.id||"").toLowerCase())>=0;const _pktV2=String(e.id||"").toLowerCase()==="aaaa1111-2222-4333-8444-202608060001";if(!_pktSplit){g_main="";g_extras="";}const _pgNew=`  <!-- ═══ PAGE 4: Produtos gerais (sem painel/porta/marcenaria) ═══ -->
  ${g_main ? `  <div class="pagemargins" style="page-break-before:always;">
  <div class="table-page">
    ...
    <table class="ptable">
      ...
      <thead>
        ...
        <tr><td class="t-label" colspan="3">${_pktV2?"APLICAÇÕES":"Itens orçados:"}</td></tr>
        <tr>
      ...
      <tbody>
        ${g_main}
        <tr class="ftotal-row"><td class="ftotal-c">SUBTOTAL</td><td class="ftotal-l"></td><td class="ftotal-v">${E(_totMain)}</td></tr>
      </tbody>
    </table>
  ...
  <!-- ═══ PAGE 5: Painel + Porta + Marcenaria ═══ -->
  ${g_extras ? `  <div class="pagemargins" style="page-break-before:always;">
  <div class="table-page">
    ...
    <table class="ptable">
      ...
      <thead>
        <tr><td class="t-cliente" colspan="3">${O}, ID: ${b}</td></tr>
        <tr><td class="t-label" colspan="3">${_pktV2?"MARCENARIA FINA":"Itens orçados:"}</td></tr>
        <tr>
          <th class="hdr-itens">ITENS</th>
          <th class="hdr-desc">DESCRITIVO</th>
          <th class="hdr-valor">VALOR</th>
      ...
      <tbody>
        ${g_extras}
        <tr class="ftotal-row"><td class="ftotal-c">SUBTOTAL</td><td class="ftotal-l"></td><td class="ftotal-v">${E(_totExtras)}</td></tr>
      </tbody>
    </table>
  ...
  <!-- ═══ PAGE 6: Valor total do orçamento + aceite ═══ -->
    <div class="pagemargins" style="page-break-before:always;">
  <div class="table-page">

    <table class="ptable ptable-summary">
      <colgroup><col class="c1"><col class="c2"><col class="c3"></colgroup>
      <tbody>
        <tr><td colspan="3" class="summary-cell">
          ${_pktV2?`<div class="summary-content pkt-nv">
            <style>/*PKT-NAVSUM-20260806 — só _pktV2 (clone teste)*/
            .slide-budget .pkt-nv{max-width:680px!important;margin:0 auto!important;padding:40px 24px 48px!important;font-family:'DM Sans',Arial,Helvetica,sans-serif!important;color:#1a1a1a!important;text-align:left!important}
            .slide-budget .pkt-nv .nv-eyebrow{font-size:11px!important;font-weight:700!important;letter-spacing:.26em!important;text-transform:uppercase!important;color:#D4A853!important;margin-bottom:34px!important;text-align:left!important}
            .slide-budget .pkt-nv .nv-resumo{display:flex!important;flex-direction:column!important;gap:12px!important;align-items:flex-end!important;text-align:right!important;padding:20px 0 24px!important;border-top:1px solid #e4e2dc!important}
            .slide-budget .pkt-nv .nv-row{display:flex!important;align-items:baseline!important;justify-content:flex-end!important;gap:20px!important}
            .slide-budget .pkt-nv .nv-label{font-size:11px!important;font-weight:600!important;letter-spacing:.16em!important;text-transform:uppercase!important;color:#6a6a6a!important;text-align:right!important}
            .slide-budget .pkt-nv .nv-sub{display:block!important;font-size:10px!important;letter-spacing:.04em!important;text-transform:none!important;color:#9a9a96!important;font-weight:400!important;margin-top:3px!important}
            .slide-budget .pkt-nv .nv-val{font-size:18px!important;font-weight:600!important;color:#1a1a1a!important;min-width:180px!important;text-align:right!important;white-space:nowrap!important}
            .slide-budget .pkt-nv .nv-row-disc .nv-label,.slide-budget .pkt-nv .nv-row-disc .nv-val{color:#9a7a4e!important;font-style:italic!important}
            .slide-budget .pkt-nv .nv-total{display:flex!important;flex-direction:column!important;align-items:flex-end!important;text-align:right!important;margin-bottom:52px!important;padding:30px 0!important;border-top:2px solid #1a1a1a!important;border-bottom:1px solid #e4e2dc!important}
            .slide-budget .pkt-nv .nv-total-label{font-size:11px!important;font-weight:600!important;letter-spacing:.28em!important;text-transform:uppercase!important;color:#6a6a6a!important;margin-bottom:10px!important}
            .slide-budget .pkt-nv .nv-total-value{font-size:44px!important;font-weight:300!important;color:#1a1a1a!important;letter-spacing:.01em!important;line-height:1.05!important}
            .slide-budget .pkt-nv .nv-cur{font-size:.45em!important;vertical-align:top!important;margin-right:6px!important;color:#6a6a6a!important;font-weight:500!important}
            .slide-budget .pkt-nv .nv-total-sub{font-size:12px!important;font-weight:400!important;color:#6a6a6a!important;margin-top:10px!important}
            .slide-budget .pkt-nv .nv-bloco{margin-bottom:44px!important}
            .slide-budget .pkt-nv .nv-h3{font-size:13px!important;font-weight:700!important;letter-spacing:.18em!important;text-transform:uppercase!important;color:#1a1a1a!important;margin:0 0 22px!important;padding-bottom:12px!important;border-bottom:1px solid #e4e2dc!important;text-align:left!important}
            .slide-budget .pkt-nv .nv-cond{display:grid!important;grid-template-columns:1fr 1fr!important;gap:22px 44px!important}
            .slide-budget .pkt-nv .nv-dt{font-size:10px!important;font-weight:600!important;letter-spacing:.22em!important;text-transform:uppercase!important;color:#6a6a6a!important;margin:0 0 6px!important;text-align:left!important}
            .slide-budget .pkt-nv .nv-dd{font-size:14px!important;font-weight:400!important;color:#1a1a1a!important;line-height:1.5!important;text-align:left!important}
            .slide-budget .pkt-nv .nv-notes{grid-column:1/-1!important}
            .slide-budget .pkt-nv .nv-bank{font-size:13px!important;color:#1a1a1a!important;line-height:1.8!important;font-weight:500!important;text-align:left!important}
            .slide-budget .pkt-nv .nv-razao{display:block!important;margin-top:4px!important;font-size:11px!important;color:#6a6a6a!important;font-weight:400!important}
            @media (max-width:640px){.slide-budget .pkt-nv .nv-total-value{font-size:32px!important}.slide-budget .pkt-nv .nv-cond{grid-template-columns:1fr!important;gap:16px!important}.slide-budget .pkt-nv .nv-val{min-width:0!important;font-size:16px!important}}
            </style>
            <div class="nv-eyebrow">Resumo financeiro</div>
            <div class="nv-resumo">
              ${_totMain>0?`<div class="nv-row"><span class="nv-label">Aplicações${_catsMain.length?`<span class="nv-sub">${_catsMain.join(" · ")}</span>`:""}</span><span class="nv-val">${E(_totMain)}</span></div>`:""}
              ${_totExtras>0?`<div class="nv-row"><span class="nv-label">Marcenaria fina${_catsExtras.length?`<span class="nv-sub">${_catsExtras.join(" · ")}</span>`:""}</span><span class="nv-val">${E(_totExtras)}</span></div>`:""}
              ${(y+_freteVal)>0?`<div class="nv-row"><span class="nv-label">Logística</span><span class="nv-val">${E(y+_freteVal)}</span></div>`:""}
              ${k>0?`<div class="nv-row nv-row-disc"><span class="nv-label">Desconto ${_dM==="valor"?"("+E(_dV)+")":"("+e.desconto_perc.toFixed(1)+"%)"}</span><span class="nv-val">&minus;&nbsp;${E(k)}</span></div>`:""}
            </div>
            <div class="nv-total">
              <span class="nv-total-label">Investimento total</span>
              <span class="nv-total-value">${E(G).replace("R$",'<span class="nv-cur">R$</span>')}</span>
              <span class="nv-total-sub">Proposta ${b} · ${O}</span>
            </div>
            <div class="nv-bloco">
              <div class="nv-h3">Condições comerciais</div>
              <div class="nv-cond">
                <div><div class="nv-dt">Pagamento</div><div class="nv-dd">${q}</div></div>
                <div><div class="nv-dt">Garantia</div><div class="nv-dd">${V}</div></div>
                <div><div class="nv-dt">Prazo de entrega</div><div class="nv-dd">${X}</div></div>
                <div><div class="nv-dt">Prazo de execução</div><div class="nv-dd">${J}</div></div>
                ${e.consideracoes?`<div class="nv-notes"><div class="nv-dt">Considerações</div><div class="nv-dd">${e.consideracoes.replace(/\n/g,"<br>")}</div></div>`:""}
              </div>
            </div>
            <div class="nv-bloco">
              <div class="nv-h3">Dados para pagamento</div>
              <div class="nv-bank">${K}<span class="nv-razao">${Q}</span></div>
            </div>
          </div>`:`<div class="summary-content">
            <div class="summary-eyebrow">Resumo do seu orçamento</div>
            <div class="summary-hero">
              <div class="hero-label">Valor total</div>
              <div class="hero-value">${E(G)}</div>
            </div>
            <div class="summary-breakdown">
              ${_totMain>0?`<div class="bd-row"><span class="bd-label"><span class="bd-title">${_pktV2?"Aplicações":"Revestimento"}</span>${_catsMain.length?`<span class="bd-sub">${_catsMain.join(" · ")}</span>`:""}</span><span class="bd-value">${E(_totMain)}</span></div>`:""}
              ${_totExtras>0?`<div class="bd-row"><span class="bd-label"><span class="bd-title">Marcenaria Fina</span>${_catsExtras.length?`<span class="bd-sub">${_catsExtras.join(" · ")}</span>`:""}</span><span class="bd-value">${E(_totExtras)}</span></div>`:""}
              ${(y+_freteVal)>0?`<div class="bd-row"><span class="bd-label"><span class="bd-title">Logística</span></span><span class="bd-value">${E(y+_freteVal)}</span></div>`:""}
              <div class="bd-row bd-total"><span class="bd-label"><span class="bd-title">Total</span></span><span class="bd-value">${E(j+y+_freteVal)}</span></div>
              ${k>0?`<div class="bd-row bd-disc"><span class="bd-label"><span class="bd-title">Desconto ${_dM==="valor"?"("+E(_dV)+")":"("+e.desconto_perc.toFixed(1)+"%)"}</span></span><span class="bd-value">- ${E(k)}</span></div>`:""}
            </div>
            <div class="summary-conditions">
              <div class="cond-title">Condições</div>
              <div class="cond-grid">
                <div class="cond-item"><div class="cond-label">Pagamento</div><div class="cond-value">${q}</div></div>
                <div class="cond-item"><div class="cond-label">Garantia</div><div class="cond-value">${V}</div></div>
                <div class="cond-item"><div class="cond-label">Prazo de entrega</div><div class="cond-value">${X}</div></div>
                <div class="cond-item"><div class="cond-label">Prazo de execução</div><div class="cond-value">${J}</div></div>
              </div>
              ${e.consideracoes?`<div class="cond-notes"><div class="cond-label">Considerações</div><div class="cond-value">${e.consideracoes.replace(/\n/g,"<br>")}</div></div>`:""}
              <div class="cond-bank">${K}<br><span class="cond-razao">${Q}</span></div>
            </div>
          </div>
        `}</td></tr>
      </tbody>
  ...
  <!-- ═══ PAGE 5+: Contract terms ═══ -->`;const _pgOld=`  <!-- ═══ PAGE 4: Items table ═══ -->
  <div class="pagemargins" style="page-break-before:always;">
  <div class="table-page">
...
${_pktSplit ? _pgNew : _pgOld}
  ...
  <div class="contract-page">
    ...
    <div class="contract-sigs-flow">
      <div style="height:32pt;line-height:32pt;text-align:center;margin-top:8pt"><span style="color:transparent;font-size:8pt;line-height:32pt;user-select:none;-webkit-user-select:none">\\sign_contratado\\</span></div><div class="sig-block" style="margin-top:0">
      ...
      <div style="height:32pt;line-height:32pt;text-align:center;margin-top:8pt"><span style="color:transparent;font-size:8pt;line-height:32pt;user-select:none;-webkit-user-select:none">\\sign_contratante\\</span></div><div class="sig-block" style="margin-top:0">
      ...
      <div style="height:32pt;line-height:32pt;text-align:center;margin-top:8pt"><span style="color:transparent;font-size:8pt;line-height:32pt;user-select:none;-webkit-user-select:none">\\sign_witness1\\</span></div><div class="sig-block" style="margin-top:0">
        <div class="sig-field">RG:</div>
        <div class="sig-field">CPF:</div>
      </div>
      <div style="height:32pt;line-height:32pt;text-align:center;margin-top:8pt"><span style="color:transparent;font-size:8pt;line-height:32pt;user-select:none;-webkit-user-select:none">\\sign_witness2\\</span></div><div class="sig-block" style="margin-top:0">
        <div class="sig-field">RG:</div>
        <div class="sig-field">CPF:</div>
      </div>
    </div>
