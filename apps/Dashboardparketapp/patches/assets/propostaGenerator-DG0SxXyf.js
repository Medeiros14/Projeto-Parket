const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/core-auth-D7HYo5n6.js","assets/vendor-react-BCRWuP_0.js","assets/vendor-supabase-4xig6v3J.js"])))=>i.map(i=>d[i]);
import{_ as D}from"./core-layout-Dl57Aqcp.js";function S(e){return e.toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}function W(e){try{return new Date(e).toLocaleDateString("pt-BR",{day:"2-digit",month:"long",year:"numeric"})}catch{return new Date().toLocaleDateString("pt-BR",{day:"2-digit",month:"long",year:"numeric"})}}function U(e){return/\s·\s/.test(e)||/·/.test(e)}function H(e){var b;const i=e.split("·").map(q=>q.trim()).filter(Boolean);return i.length>=2?i[1].toUpperCase():((b=i[0])==null?void 0:b.toUpperCase())||""}function z(e){const i=(e||"").toUpperCase();return i.startsWith("INSUMOS DE")||i.includes(`
INSUMOS DE`)||/^INSUMOS\b/.test(i)?1:i.startsWith("INSTALAÇÃO")||i.includes("INSTALAÇÃO E GEST")?2:0}function Z(e,i){const b=i.reduce((o,t)=>(o[t.categoria]||(o[t.categoria]=[]),o[t.categoria].push(t),o),{}),q=W(e.created_at||new Date().toISOString()),T=e.numero&&String(e.numero).trim()||String(Date.now()).slice(-4),f=e.cliente||"—",R=e.cnpj_cpf||"—",r=e.endereco||"—",n=e.vendedor||"—",a=e.orcamentista||"",p=e.contato_nome||"",c=e.contato_telefone||"",s=e.contato_email||"",v=e.arquiteto||"",$=e.forma_pagamento||"A combinar",C=v?`${f} | ${v}`:f,L=e.vendedor_telefone||"011 98675-0031",F=e.vendedor_email||"douglas@parket.com.br",M=r.split(`
`).filter(Boolean);let I=0,y="";const N={};for(const o of Object.keys(b)){const t=o.toUpperCase();t.includes("LOG")||t==="TRANSPORTE"?I=b[o].reduce((u,O)=>u+O.valor,0):t.includes("OBS")?y=b[o].map(u=>u.descritivo).join("; "):N[o]=b[o]}const w=Object.values(N).flat().reduce((o,t)=>o+t.valor,0),P=w*(e.desconto_perc/100),G=w+I-P;if(!y){const o=["Fornecimento e instalação"];for(const t of Object.keys(N)){const[u,O,_]=t.split("||"),j=N[t];let x=0;for(const l of j){const m=(l.descritivo||"").toUpperCase();if(m.includes("INSUMOS")||m.includes("INSTALAÇÃO"))continue;const g=(l.descritivo||"").match(/Metragem real ([0-9]+[.,]?[0-9]*)m²/i);g&&(x+=parseFloat(g[1].replace(",",".")))}const d=O?_?` ${O} ${_}`:` ${O}`:"";x>0?o.push(`${x.toFixed(2)}m² - ${u.toUpperCase()}${d}`):o.push(`${u.toUpperCase()}${d}`)}y=o.join(`
`)}let A="",k=1;for(const o of Object.keys(N)){const t=[...N[o]].sort((d,l)=>{const m=z(d.descritivo),g=z(l.descritivo);return m!==g?m-g:d.ordem-l.ordem}),u=t.reduce((d,l)=>d+l.valor,0),[O,_]=o.split("||"),j=O;let x=_||"";if(!x)for(const d of t){const l=(d.descritivo||"").split(`
`).find(U);if(l){x=H(l);break}}A+=`<tr>
      <td class="cat-c">${j.toUpperCase()}</td>
      <td class="cat-l">${x.replace(/_/g," ")}</td>
      <td class="cat-v"></td>
    </tr>`,t.forEach((d,l)=>{const m=(d.descritivo||"").split(`
`).map(E=>E.trim()).filter(Boolean).filter(E=>!U(E)&&!/^__RECDATA__:/.test(E)).map(E=>E.replace(/[a-záéíóúç]+_[a-záéíóúç_]+/gi,Q=>Q.replace(/_/g," ")));let g="";m.length>1?(g+=`<p class="desc-title">${m[0]}</p>`,g+=`<p class="desc-body">${m.slice(1).join('</p><p class="desc-body">')}</p>`):m.length===1?g+=`<p class="desc-title">${m[0]}</p>`:g+=`<p class="desc-title">${d.descritivo}</p>`,A+=`<tr>
        <td class="item-c">${k}.${l+1}</td>
        <td class="item-l">${g}</td>
        <td class="item-v">${S(d.valor)}</td>
      </tr>`}),A+=`<tr>
      <td class="sub-c"></td>
      <td class="sub-l">Valor parcial - ${j.toLowerCase()} </td>
      <td class="sub-v">${S(u)}</td>
    </tr>`,k++}I>0&&(A+=`<tr>
      <td class="log-c">LOGÍSTICA</td>
      <td class="log-l">TRANSPORTE, DESLOCAMENTO, HOSPEDAGEM, E ALIMENTAÇÃO DA EQUIPE</td>
      <td class="sub-v">${S(I)}</td>
    </tr>`),A+=`<tr>
    <td class="obs-c">OBSERVAÇÃO</td>
    <td class="obs-l">${y.replace(/\n/g,"<br>")}</td>
    <td class="obs-v"></td>
  </tr>`,A+=`<tr>
    <td class="total-c"></td>
    <td class="total-l">RESUMO TOTAL DOS PRODUTOS ORÇADOS</td>
    <td class="total-v">${S(w)}</td>
  </tr>`,e.desconto_perc>0&&(A+=`<tr>
      <td class="disc-c"></td>
      <td class="disc-l">&nbsp;DESCONTO CONCEDIDO (${e.desconto_perc.toFixed(1)}%)</td>
      <td class="disc-v">-${S(P)}</td>
    </tr>`),A+=`<tr class="ftotal-row">
    <td class="ftotal-c">TOTAL</td>
    <td class="ftotal-l"></td>
    <td class="ftotal-v">${S(G)}</td>
  </tr>`;const B=e.pag_garantia||"10 anos",V=e.pag_prazo_entrega||"120 dias após a contratação",X=e.pag_prazo_execucao||"120 dias após a entrega do material",J=e.pag_dados_bancarios||"Banco Itaú | Agencia 3720 CC 30.288-8 | PIX: pamella@parket.com.br",K=e.pag_razao_social||"Mundial Export Assess. Com. e Ext. Imp e Exp Eireli | CNPJ 29.872.616/0001-34";A+=`<tr>
    <td class="cond-c">CONDIÇÕES</td>
    <td class="cond-l">
      <p><b>Condições de pagamento: </b>${$}</p>
      <p><b>Garantia: </b>${B}</p>
      <p><b>Prazo de entrega: </b>${V}</p>
      <p><b>Prazo de execução: </b>${X}</p>
      <p><b>Dados bancários: </b>${J}</p>
      <p>${K}</p>
    </td>
    <td class="cond-v"></td>
  ...
  <div class="page-content">
    <div class="page-header-right">${C}, ID: ${T}</div>`,e.composicao_faturamento&&(h+=`
    ...
    </div>`);const o=e.remetente_nome||"",t=e.remetente_email||"",u=e.remetente_telefone||"";o&&(h+=`
...
<body>
  ...
  <div class="page-fixed info-page">
    ...
    <div class="info-block">
      <div class="info-line">São Paulo, ${q}</div>

      <div class="info-gap"></div>
      <div class="info-line">Proposta comercial - ${T}</div>

      <div class="info-gap"></div>
      <div class="info-line">Contratante: ${C}</div>
${M.map(o=>`      <div class="info-line">${o}</div>`).join(`
`)}${p?`
      ...
      <div class="info-line">Vendedor: ${n}</div>${a?`
      <div class="info-line">Orçamentista: ${a}</div>`:""}
  ...
  <div class="table-page">
    ...
    <table class="ptable">
      ...
      <thead>
        <tr><td class="t-cliente" colspan="3">${C}, ID: ${T}</td></tr>
...
</html>`}async function ee(e,i){if(e.obra_id)try{const{supabase:r}=await D(async()=>{const{supabase:p}=await import("./core-auth-D7HYo5n6.js").then(c=>c.a);return{supabase:p}},__vite__mapDeps([0,1,2])),{data:n}=await r.from("kanban_cards").select("details").eq("id",e.obra_id).single(),a=(n==null?void 0:n.details)??{};e={...e,cnpj_cpf:e.cnpj_cpf||a.cnpj_cpf||"",endereco:e.endereco||a.fax||a.endereco_obra||"",vendedor:a.vendedor||e.vendedor||"",orcamentista:a.orcamentista||e.orcamentista||"",vendedor_email:a.outro_email||e.vendedor_email||"",vendedor_telefone:a.vendedor_telefone||a.celular||a.telefone_comercial||e.vendedor_telefone||"",contato_nome:a.contato_principal||"",contato_telefone:a.celular||a.telefone_comercial||a.tel_direto_comercial||"",contato_email:a.email_comercial||a.email_pessoal||"",forma_pagamento:a.forma_pagamento||e.forma_pagamento||"A combinar",pag_garantia:a.pag_garantia||"",pag_prazo_entrega:a.pag_prazo_entrega||"",pag_prazo_execucao:a.pag_prazo_execucao||"",pag_dados_bancarios:a.pag_dados_bancarios||"",pag_razao_social:a.pag_razao_social||""}}catch(r){console.warn("Falha ao puxar dados do card:",r)}if(!e.numero||!String(e.numero).trim())try{const{supabase:r}=await D(async()=>{const{supabase:s}=await import("./core-auth-D7HYo5n6.js").then(v=>v.a);return{supabase:s}},__vite__mapDeps([0,1,2])),{data:n}=await r.from("simulacao_projetos").select("numero"),a=(n??[]).map(s=>parseInt(String(s.numero??"").replace(/\D/g,""),10)).filter(s=>!isNaN(s)).reduce((s,v)=>Math.max(s,v),1499),p=String(a+1);e={...e,numero:p};const c=e.id;c&&await r.from("simulacao_projetos").update({numero:p}).eq("id",c)}catch(r){console.warn("Falha ao auto-gerar número da proposta:",r),e={...e,numero:String(Date.now()).slice(-4)}}try{const{supabase:r}=await D(async()=>{const{supabase:a}=await import("./core-auth-D7HYo5n6.js").then(p=>p.a);return{supabase:a}},__vite__mapDeps([0,1,2])),{data:n}=await r.from("orcamento_tabela_precos").select("especie_nome, dimensao_obs, tipo_porta, categoria").eq("ativo",!0).not("dimensao_obs","is",null);n&&n.length>0&&(i=i.map(a=>{const p=(a.descritivo||"").toLowerCase(),c=n.find(s=>s.dimensao_obs&&s.dimensao_obs.trim().length>10&&p.includes((s.especie_nome||"").toLowerCase()));if(c!=null&&c.dimensao_obs){const s=c.dimensao_obs.split(/,\s*/).map(v=>v.trim()).filter(Boolean).map(v=>`• ${v}`).join(`
`);return{...a,descritivo:`${a.descritivo}

Ferragens e Insumos:
${s}`}}return a}))}catch(r){console.warn("Falha ao buscar ferragens pra enriquecer PDF:",r)}const b=typeof window<"u"?window.location.origin:"",q=b+"/";let T=Z(e,i).replace("<head>",`<head>
<base href="${q}">`);T=T.replace(/src="\/proposta-cover\.png"/g,`src="${b}/proposta-cover.png"`);const f=window.open("","_blank","width=900,height=700");if(!f){alert("Permita pop-ups para gerar a proposta.");return}f.document.write(T),f.document.close(),f.focus();const R=()=>{const r=Array.from(f.document.images);r.length===0||r.every(n=>n.complete&&n.naturalWidth>0)?f.print():setTimeout(R,200)};setTimeout(R,300)}export{ee as a,Z as g};
