...
export type NfeData = {
  ...
  fornecedorRazao: string;    // xNome (razão social oficial da NFe)
  fornecedorTelefone: string; // fone do emitente
  fornecedorEmail: string;    // email do emitente (raro em NFe)
  fornecedorEndereco: string; // xLgr + nro + xBairro + xMun/UF
  valorTotal: number;
  itens: NfeItem[];
  parcelas: NfeParcela[];
};
...
export function parseNfeXml(xmlText: string): NfeData {
  ...
  // Endereço + fone do emitente (tag enderEmit)
  const enderEmit = emit.getElementsByTagName("enderEmit")[0];
  const endereco = enderEmit
    ? [texto(enderEmit, "xLgr"), texto(enderEmit, "nro"), texto(enderEmit, "xBairro"),
       (texto(enderEmit, "xMun") ? `${texto(enderEmit, "xMun")}/${texto(enderEmit, "UF")}` : "")]
      .filter(Boolean).join(", ")
    : "";
  const telefone = enderEmit ? texto(enderEmit, "fone") : "";
  const xNome = texto(emit, "xNome");
  const xFant = texto(emit, "xFant");

  return {
    chaveAcesso,
    numeroNf: ide ? texto(ide, "nNF") : "",
    dataEmissao,
    fornecedorNome: xFant || xNome,   // fantasia preferido, senão razão
    fornecedorCnpj: texto(emit, "CNPJ") || texto(emit, "CPF"),
    fornecedorRazao: xNome,
    fornecedorTelefone: telefone,
    fornecedorEmail: texto(emit, "email"),
