...
function CardItem({ card, demanda, orcamentistaNome, onAction, onDragStart, onDragEnd }) {
  ...
  return c("div", {
    ...
    c("div", { style: { display: "flex", gap: 6, fontSize: "0.5rem", color: TEXT_DIM, alignItems: "center", flexWrap: "wrap" } },
      ...
      // Quando card está em coluna de conclusão (proposta-pronta / handoff-com / proposta-aceita / refazer-recusado)
      // o countdown não faz sentido — mostra "Finalizado" em vez de "Atrasado".
      (() => {
        const _doneCols = ["proposta-pronta","handoff-com","proposta-aceita"];
        const _isDone = _doneCols.indexOf(card.column_id) >= 0;
        if (_isDone) {
          return c("span", { style: { background: `${GREEN}25`, color: GREEN, padding: "3px 8px", borderRadius: 10, fontWeight: 700, fontSize: "0.7rem" } }, "✅ Finalizado");
        }
        if (demanda && demanda.aceito_em && demanda.prazo_horas) {
          return c(Countdown, { aceitoEm: demanda.aceito_em, prazoHoras: demanda.prazo_horas });
        }
        return prazoStr && c("span", { style: { background: `${ACCENT}25`, padding: "3px 8px", borderRadius: 10, color: ACCENT, fontWeight: 700, fontSize: "0.7rem" } }, `⏱ Prazo: ${prazoStr}`);
      })(),
    ),
