...
const ESPECIES = [
  ...
  "Peroba do Campo", "Cabreuva Branca", "Cabreuva Dourada", "Imbuia", "Tauari Customizado",
];
...
function MarcenariaPersonalizadaTab() {
  ...
  const ferBlock = c("div", { key: "f", style: S.card },
    ...
    showAddFer && c("div", {
      style: {
        display: "grid", gridTemplateColumns: "2fr 1fr auto", gap: 8, marginBottom: 12,
        padding: 12, background: "rgba(34,197,94,0.05)", borderRadius: 8, border: "1px solid rgba(34,197,94,0.2)",
      },
    },
      c("input", {
        value: novaFerNome, onChange: (e) => setNovaFerNome(e.target.value),
        placeholder: "Nome da ferragem nova", style: S.field,
      }),
      c("input", {
        value: novaFerForn, onChange: (e) => setNovaFerForn(e.target.value),
        placeholder: "Fornecedor (opcional)", style: S.field,
      }),
      c("button", {
        type: "button", onClick: adicionarNovaFerragem,
        disabled: !novaFerNome.trim(),
        style: { ...S.btnPrim, opacity: !novaFerNome.trim() ? 0.5 : 1, padding: "10px 14px", fontSize: 12 },
      }, "Salvar"),
    ),
    loadingFer
