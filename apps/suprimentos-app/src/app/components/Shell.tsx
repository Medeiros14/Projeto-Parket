...
import {
  HardHat, Package, Plus, KanbanSquare,
  Wrench, Users, Database, FileClock,
  Sun, Moon, LogOut,
 ...
 */
const NAV = [
  { to: "/",             icon: HardHat,      label: "Em Uso (Rua)" },
  { to: "/estoque",      icon: Package,      label: "Estoque" },
  { to: "/manutencao",   icon: Wrench,       label: "Manutenção" },
  { to: "/entrada",      icon: Plus,         label: "Entrada / Baixas" },
  { to: "/kanban",       icon: KanbanSquare, label: "Kanban" },
  { to: "/funcionarios", icon: Users,        label: "Funcionários" },
  { to: "/historico",    icon: FileClock,    label: "Histórico de Termos" },
  { to: "/dados",        icon: Database,     label: "Dados e Excel" },
];
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  const [collapsed, setCollapsed] = useState<boolean>(() => {
    ...
    return localStorage.getItem("suprimentos-sidebar-collapsed") === "1";
  });
  useEffect(() => {
    try { localStorage.setItem("suprimentos-sidebar-collapsed", collapsed ? "1" : "0"); } catch { /* noop */ }
  }, [collapsed]);
      ...
      >
        ...
        <div className={`${collapsed ? "p-2 justify-center" : "p-4"} border-b border-va-border flex items-center gap-2`}>
          <div className="w-8 h-8 bg-va-accent/15 border border-va-accent/40 flex items-center justify-center shrink-0">
            <span className="font-display text-va-accent text-[13px] leading-none">S</span>
          </div>
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <div className="font-display font-medium text-[13px] leading-tight tracking-[0.16em] text-va-text">PARKET</div>
              <div className="text-[8px] uppercase tracking-[0.22em] text-va-textDim mt-1">SUPRIMENTOS · FERRAMENTAS</div>
            </div>
          )}
          {!collapsed && (
            <button
