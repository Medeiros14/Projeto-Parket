...
(function () {
  ...
  function findCardAncestor(node) {
    var cur = node;
    for (var d = 0; d < 8; d++) {
      if (!cur || cur === document.body || !cur.getBoundingClientRect) return null;
      try {
        var rect = cur.getBoundingClientRect();
        if (rect.height >= 40 && rect.height <= 320 && rect.width >= 120 && rect.width <= 600) {
          var cs = getComputedStyle(cur);
          var clickable = cs.cursor === "pointer" || cur.onclick || cur.getAttribute("role") === "button" || /draggable|card|item|tile/i.test(cur.className || "");
  ...
  // Realtime global pra reagir na hora
  try {
    openGlobalChatRealtime(function (rec) {
      var me = ckUserEmail();
      if (!me) return;
      if (rec && rec.user_email && String(rec.user_email).toLowerCase() === me) return; // self
      ...
      if (!unreadByContext[cid]) {
        showToast(rec, cid);
        unreadByContext[cid] = { count: 1, topic: rec.topic, lastMsg: rec };
      } else {
        unreadByContext[cid].count++;
        unreadByContext[cid].lastMsg = rec;
      }
      if (rec.topic) unreadByTopic[rec.topic] = (unreadByTopic[rec.topic] || 0) + 1;
