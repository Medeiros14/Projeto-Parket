        ...
        await streamResponse({
          ...
          onChunk: (chunk: RunResponse) => {
            if (
              ...
              // HITL: run pausada aguardando confirmação de tool
              setMessages((prevMessages) => {
                const newMessages = [...prevMessages]
                const lastMessage = newMessages[newMessages.length - 1]
                if (
                  ...
                  lastMessage.paused = {
                    runId: chunk.run_id,
                    agentId: (chunk.agent_id as string) ?? agentId ?? '',
                    sessionId:
                      (chunk.session_id as string) ?? newSessionId ?? '',
                    tools: (chunk.tools ?? []).filter(
