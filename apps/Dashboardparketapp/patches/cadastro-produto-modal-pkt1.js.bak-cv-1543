...
const MODELOS_POR_TIPO_PORTA = {
  "Porta de Passagem": ["CIR", "GERIS", "3D", "Sob Medida"],
  "Porta Pivotante":   ["CIR", "GERIS", "Italy Line", "Tectus", "Sob Medida"],
  "Porta de Correr":   ["DN150", "RO82TOP", "Sob Medida"],
  "Porta Camarão":     ["CIR", "2 Folhas", "Sob Medida"],
};
const CATEGORY_CONFIG = {
  ...
  porta: {
    ...
    fields: [
      ...
      { key: "acabamento", label: "Acabamento", options: ["Laca", "Lâmina Natural", "Ripado", "Muxarabi", "Moldura Vidro", "Blindada", "Maciço"] },
...
<div id="__pkt_cad" style="position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;padding:24px;">
  <div style="background:${CARD_BG};border:1px solid ${BORDER};border-radius:14px;width:min(820px,100%);max-height:90vh;overflow-y:auto;padding:24px;color:${TEXT};font-family:system-ui,sans-serif;">
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
      ...
      <!-- Espécie (combobox) -->
      <div>
        <label style="font-size:0.55rem;color:${TEXT_DIM};text-transform:uppercase;letter-spacing:0.05em;display:block;margin-bottom:4px;">Espécie *</label>
        <input id="__pkt_especie" list="__pkt_especie_list" placeholder="ex: Tauari" style="${_inputStyle()}" />
        <datalist id="__pkt_especie_list"></datalist>
        <p style="font-size:0.55rem;color:${TEXT_DIM};margin:3px 0 0;">Digite um valor novo ou escolha da lista (valores já cadastrados)</p>
      </div>

      <!-- Campos específicos da categoria — preenchidos por JS -->
...
<div>
  ...
  $("__pkt_save").onclick = async () => {
    ...
    const payload = {
      ...
      subtipo: (function(){ const v=(atributos_extras.subtipo||"").toLowerCase().replace(/\s+/g,"_"); const isRegua=v===""||v==="regua"||v==="régua"||v==="réguas"||v==="reguas"; if(!isRegua) return v; return (initial.categoria==="painel"||initial.categoria==="revestimento")?"reguas":"regua"; })(),
      dimensao_id: initial.dimensao_id || (atributos_extras.dimensao || "default")
        .toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""),
      dimensao_label: atributos_extras.dimensao || "—",
    ...
    if (cat === "porta") {
      ...
      // 'modelo' = "CIR" (marca curta) → tipo_porta composto: "Pivotante CIR"
      payload.modelo_porta = atributos_extras.tipo || null;
      const _tipoShort = (atributos_extras.tipo || "").replace(/^Porta (de |da |do )?/i, "").trim();
      const _m = (atributos_extras.modelo || "").trim();
      payload.tipo_porta = (_tipoShort && _m) ? `${_tipoShort} ${_m}` : (_m || _tipoShort || null);
    }
    ...
    try {
      ...
      if (result.error) {
        console.error("[CadProduto] supabase error:", result.error, "payload:", payload);
