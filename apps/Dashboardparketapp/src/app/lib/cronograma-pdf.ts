/**
 * cronograma-pdf.ts
 * Gera PDF de acompanhamento de obras (cronograma de serviços)
 * com layout paisagem A4 estilo Parket — capa preta, tabelas, alertas,
 * prazo de entrega e registro fotográfico.
 *
 * Reverse-engineered from golden build: cronograma-pdf-NEW2026.js
 */
...
/** Formata data ISO → pt-BR (dd/mm/yyyy). */
...
async function imageToDataUri(url: string): Promise<string> {
  // Tenta via fetch + createImageBitmap primeiro
  try {
...
const OBS_PADRAO =
  "O PRAZO PARA EXECUÇÃO DOS SERVIÇOS PELA PARKET PASSA A SER CONTABILIZADO " +
  "SOMENTE APÓS A LIBERAÇÃO DA OBRA PELO FISCAL RESPONSÁVEL E A DEVIDA APROVAÇÃO DO PROJETO.";
...
export async function gerarCronogramaPdf(n: CronogramaPdfInput): Promise<void> {
   ...
   * (decisão do produto). NÃO RE-ADICIONAR sem alinhar com o time —
   * esse PDF vai pro cliente e a coluna observação tem dados internos.
   * 6 colunas fixas: Descrição (largura flexível), 5 colunas de dados
   * iguais (30mm cada). */
  ...
  for (let idx = 0; idx < n.itens.length; idx++) {
    ...
    // Altura fixa da linha (sem coluna observação que esticava)
    const rh = 14;
    ...
    // Col 2 — Instalado
    doc.text(
      `${item.instalado ?? 0} ${item.unidade.toUpperCase()}`,
