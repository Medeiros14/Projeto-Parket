...
`)}let g="",U=1;const z=o=>/\s·\s/.test(o)||/·/.test(o),B=o=>{var n;const t=o.split("·").map(v=>v.trim()).filter(Boolean);return t.length>=2?t[1].toUpperCase():((n=t[0])==null?void 0:n.toUpperCase())||""},$=o=>{const t=(o||"").toUpperCase();return t.startsWith("RECORTES")?0.5:t.startsWith("INSUMOS DE")||t.includes(`
INSUMOS DE`)||t.match(/^INSUMOS\b/)?1:t.startsWith("INSTALAÇÃO")||t.includes("INSTALAÇÃO E GEST")?2:0};const _sortedCatKeys=Object.keys(x).sort((a,b)=>{const au=a.split("||")[0].toUpperCase(),bu=b.split("||")[0].toUpperCase();const aIsRod=au==="RODAPÉ"||au==="RODAPE"?1:0,bIsRod=bu==="RODAPÉ"||bu==="RODAPE"?1:0;if(aIsRod!==bIsRod)return aIsRod-bIsRod;const _catOrder=["PISO","PAINEL","FORRO","DECK","PORTA","ESCADA","LOGISTICA","LOGÍSTICA","COMPRAS","INSUMOS"];const _ai=_catOrder.indexOf(au),_bi=_catOrder.indexOf(bu);if(au===bu)return 0;if(_ai===-1&&_bi===-1)return au.localeCompare(bu);if(_ai===-1)return 1;if(_bi===-1)return-1;return _ai-_bi});const _ambByOrdem={};for(const _cat of Object.keys(x)){if(_cat.toUpperCase().startsWith("RODAP"))continue;for(const _it of x[_cat]){const _l=(_it.descritivo||"").toUpperCase();if(_l.includes("INSUMOS")||_l.includes("INSTALAÇÃO"))continue;const _fl=(_it.descritivo||"").split("\n")[0].trim();if(_fl&&!_ambByOrdem[_it.ordem])_ambByOrdem[_it.ordem]=_fl;}}let _allObsPorProduto=[];for(const o of _sortedCatKeys){const _agg=(()=>{const _arr=x[o];const _map=new Map();const _out=[];const _isMarc=o.toUpperCase().startsWith("MARCENARIA");let _marcExtraTotal=0;for(const _it of _arr){const _d=(_it.descritivo||"").toUpperCase().trim();const _key=_d.startsWith("INSUMOS DE")?"_INSUMOS":(_d.startsWith("INSTALA")?"_INSTALACAO":null);if(_key){if(_isMarc){_marcExtraTotal+=Number(_it.valor||0);continue;}if(_map.has(_key)){const _ex=_map.get(_key);_ex.valor=Number(_ex.valor||0)+Number(_it.valor||0);}else{const _copy={..._it};_map.set(_key,_copy);_out.push(_copy)}}else{_out.push(_it)}}if(_isMarc&&_marcExtraTotal>0&&_out.length>0){const _baseTotal=_out.reduce((s,it)=>s+Number(it.valor||0),0);if(_baseTotal>0){_out.forEach(it=>{const _share=Number(it.valor||0)/_baseTotal;it.valor=Number(it.valor||0)+_marcExtraTotal*_share;});}else{_out[0].valor=Number(_out[0].valor||0)+_marcExtraTotal;}}return _out;})();const t=[..._agg].sort((d,l)=>{const u=$(d.descritivo),A=$(l.descritivo);if(u<=0.5&&A<=0.5){if(d.ordem!==l.ordem)return d.ordem-l.ordem;return u-A;}return u!==A?u-A:d.ordem-l.ordem;}),n=t.reduce((d,l)=>d+l.valor,0),__cParts=o.split("||"),v=__cParts[0],P=__cParts[1]||"",C=v;let N=P||"";if(C.toUpperCase()==="RODAPÉ"||C.toUpperCase()==="RODAPE"){const _firstRod=t.find(d=>!(d.descritivo||"").toUpperCase().startsWith("INSUMOS")&&!(d.descritivo||"").toUpperCase().startsWith("INSTALAÇÃO"));if(_firstRod){const _rl=(_firstRod.descritivo||"").split("\n").filter(l=>l.trim()&&!/^RODAPÉ$/i.test(l.trim()));if(_rl.length>0){const _nm=_rl[0].replace(/^[\d.,]+\s*mtl?\s*/i,"").replace(/\s*RODAP[ÉE]\s*/i,"").replace(/\s*—.*$/,"").replace(/\s*20X20XCOMP\.VARIAVEL\s*/i,"").trim();if(_nm)N=_nm.toUpperCase()}}};if(!N)for(const d of t){const l=(d.descritivo||"").split(`
`).find(z);if(l){N=B(l);break}}if((C.toUpperCase()==="PAINEL"||C.toUpperCase()==="REVESTIMENTO")&&/LAMIN/i.test(N)){N=N.replace(/\s*\d+\/\d+\s*[×x]\s*\d+\s*MM?/gi,"").replace(/\s+/g," ").trim();}g+=`<tr>
      <td class="cat-c">${C.toUpperCase()} </td>
      <td class="cat-l">${(function(_x){var _w=/^(PISO|FORRO|PAINEL|REVESTIMENTO)$/i.test(C);var _r=(__cParts[2]||"").trim();_r=_r.replace(/COMP\.\s*VARI[ÁA]VEL/gi,"1000 a 3000mm");if(!_w||!_x||!_r)return _x;return "LÂMINA "+_x+"<br>"+_r.toUpperCase();})((C.toUpperCase()==="PORTA"?N.replace(/\b(?:DN150|RO82TOP|CIR|GERIS|ITALY\s+LINE|3D)\b/gi," ").replace(/\b(ASSOALHO|LACA|LAMINA|MOLDURA[_ ]VIDRO|MUXARABI|RIPADO|TOBLERONE)\b\s+(?=\S)/gi," ").replace(/\s+/g," ").trim().split(" ").filter(Boolean).filter((_v,_i,_a)=>_i===0||_v.toUpperCase()!==_a[_i-1].toUpperCase()).join(" ").replace(/_/g," "):((function(){var _t=N.replace(/_/g," "),_c=C.toUpperCase();if(_c==="MARCENARIA"){return _t.split(/\s*\|\s*/).map(function(s){return s.trim().toUpperCase()}).filter(Boolean).join(" / ");}var _isLam=(_c==="PAINEL"||_c==="REVESTIMENTO")&&_t&&!/LAMINA|RIPADO|TOBLERONE|DECK|MACI[ÇC]O|MOLDURA|MUXARABI|DEMOLI[ÇC][AÃ]O|MULTIBLOC|LACA|ASSOALHO|R[ÉE]GUA/i.test(_t);return _t.replace(/\s+/g," ").trim().split(" ").filter(Boolean).filter((_v,_i,_a)=>_i===0||_v.toUpperCase()!==_a[_i-1].toUpperCase()).join(" ")})())))}</td>
      <td class="cat-v"></td>
    </tr>`;let _recIdx=0,_ambSum=0,_ambCount=0,_subtotalAdded=false,_rowIdx=0;t.forEach((d,l)=>{const _hdU=((d.descritivo||"").split("\n")[0]||"").trim().toUpperCase();const _isAmbItem=!/^INSTALA|^INSUMOS|^RECORTES/.test(_hdU);if(_isAmbItem){_ambSum+=(+d.valor||0);_ambCount++;}else if(!_subtotalAdded&&_ambCount>=2){_rowIdx++;g+=`<tr><td class="item-c">${U+"."+_rowIdx}</td><td class="item-l" style="font-weight:bold">SUBTOTAL AMBIENTES</td><td class="item-v">${E(_ambSum)}</td></tr>`;_subtotalAdded=true;}const _isRec=(d.descritivo||"").toUpperCase().startsWith("RECORTES");const u=(d.descritivo||"").split(`
`).map(I=>I.trim()).filter(Boolean).filter(I=>!z(I)&&!/^__RECDATA__:/.test(I)&&!/^\s*\d+\s+unidade(s)?\s*$/i.test(I)&&!((C.toUpperCase()==='PAINEL'||C.toUpperCase()==='REVESTIMENTO'||C.toUpperCase()==='FORRO')&&/^\s*\d+\/\d+\s*[×x]\s*\d+\s*(MM)?\s*$/i.test(I))&&!(C.toUpperCase()==='MARCENARIA'&&(/^\s*\(inclui ferragens/i.test(I)||/^\s*Madeiras:/i.test(I)||/^\s*Metragem (real|total)/i.test(I)))).map(I=>I.replace(/[a-záéíóúç]+_[a-záéíóúç_]+/gi,W=>W.replace(/_/g," "))).map(function(I){var _hd=((d.descritivo||"").split("\n")[0]||"").trim().toUpperCase();if(/^INSUMOS|^RECORTES/.test(_hd))return I;if(/^INSTALA/.test(_hd)){if(/^Metragem\s+real\s+total\b/i.test(I)){return I.replace(/^Metragem\s+real\s+total\b/i,"Metragem total");}var _mPi=I.match(/^Metragem\s+total\s+com\s+perda\s+de\s+([0-9]+)\s*%\s*([0-9]+(?:[.,][0-9]+)?)\s*m²\s*$/i);if(_mPi){var _pcti=parseFloat(_mPi[1]);var _wPi=parseFloat(_mPi[2].replace(",","."));var _real=Math.round(_wPi/(1+_pcti/100));return "Metragem total "+_real+"m²";}return I;}var _mP=I.match(/^Metragem\s+real(?:\s+total)?\s+[0-9]+(?:[.,][0-9]+)?\s*m²\s*\+\s*([0-9]+)\s*%\s*de\s+perda\s*=\s*([0-9]+(?:[.,][0-9]+)?)\s*m²\s*$/i);if(_mP){return "Metragem total com perda de "+_mP[1]+"% "+_mP[2]+"m²";}var _mm=I.match(/^Metragem\s+real(?:\s+total)?\s+([0-9]+(?:[.,][0-9]+)?)\s*m²\s*$/i);if(!_mm)return I;var _vv=parseFloat(_mm[1].replace(",","."));if(!isFinite(_vv)||_vv<=0)return I;var _vp=Math.round(_vv*1.10).toLocaleString("pt-BR");return "Metragem total com perda de 10% "+_vp+"m²";});let A="";const _isPortaCat=C.toUpperCase()==="PORTA";if(u.length>1){A+=`<p class="desc-title">${u[0]}</p>`;A+=`<p class="desc-body">${u.slice(1).join('</p><p class="desc-body">')}</p>`;}else if(u.length===1){A+=`<p class="desc-title">${u[0]}</p>`;}else{A+=`<p class="desc-title">${d.descritivo}</p>`;}if(d.cor&&!_isRec&&!/^(INSUMOS|INSTALA)/i.test((d.descritivo||"").trim())){A+=`<p class="desc-body" style="color:#666">Cor: ${d.cor}</p>`;}if(C.toUpperCase()==="MARCENARIA"){const _accs=N.replace(/_/g," ").split(/\s*\|\s*/).map(function(s){return s.trim()}).filter(Boolean);if(_accs.length>=1){const _ext=_accs[0]||"";const _int=_accs[1]||"";let _accHtml=`<p class="desc-body" style="margin-top:6pt"><strong>Acabamentos:</strong></p>`;if(_ext)_accHtml+=`<p class="desc-body">• <strong>Externo:</strong> ${_ext}</p>`;if(_int)_accHtml+=`<p class="desc-body">• <strong>Interno:</strong> ${_int}</p>`;A+=_accHtml;}}if(C.toUpperCase().startsWith("RODAP")){const _amb=_ambByOrdem[d.ordem];const _mtlM=(d.descritivo||"").match(/^\s*([0-9]+[.,]?[0-9]*)\s*mtl/i);const _mtlV=_mtlM?_mtlM[1]:null;if(_amb&&_mtlV){A=`<p class="desc-title">${_amb}</p>`+`<p class="desc-body">metragem total ${_mtlV}mtl</p>`;}}const _itemLabel=U+"."+(++_rowIdx);g+=`<tr>
        <td class="item-c">${_itemLabel}</td>
        <td class="item-l">${A}</td>
        <td class="item-v">${E(d.valor)}</td>
      </tr>`}),((function(){var _op=(t[0]&&t[0].meta&&t[0].meta.observacao_produto||"").toString().trim();if(!_op){try{_op=(_obsAuto&&_obsAuto[o]?String(_obsAuto[o]):"").trim();}catch(_e){_op="";}}if(_op){g+=`<tr><td class="obs-c">OBSERVAÇÃO</td><td class="obs-l">${_op.replace(/\n/g,"<br>")}</td><td class="obs-v"></td></tr>`;_allObsPorProduto.push(`<strong>${C.toUpperCase()}${N?" "+N:""}:</strong> ${_op}`);}})()),g+=`<tr>
      <td class="sub-c"></td>
      <td class="sub-l">Valor parcial do ${C.toUpperCase()}${N?" "+N:""} </td>
    ...
    <td class="obs-c">RESUMO</td>
    <td class="obs-l">${(_allObsPorProduto.length>0?_allObsPorProduto.join("<br><br>"):w).replace(/\n/g,"<br>")}</td>
    <td class="obs-v"></td>
  </tr>`,g+=`<tr>
    <td class="total-c"></td>
    <td class="total-l">RESUMO TOTAL DOS PRODUTOS ORÇADOS</td>
    <td class="total-v">${E(j)}</td>
  </tr>`,e.desconto_perc>0&&(g+=`<tr>
      <td class="disc-c"></td>
      <td class="disc-l">&nbsp;DESCONTO CONCEDIDO (${e.desconto_perc.toFixed(1)}%)</td>
...
<style>
/* ─── Reset & Base ─── */
* { margin:0; padding:0; box-sizing:border-box; font-synthesis:none; font-style:normal; }
html, body { background:#ededed; font-style:normal !important; font-family:Arial, Helvetica, sans-serif; -webkit-print-color-adjust:exact !important; print-color-adjust:exact !important; }
...
.cover-logo {
  ...
  font:300 34pt Arial, Helvetica, sans-serif;
...
.cover-photo-watermark {
  ...
  font:300 14pt Arial, Helvetica, sans-serif;
...
.item-l .desc-body {
  margin:0 0 5pt 0; font-size:8pt; line-height:1.35;
  font-weight:normal;
}
.item-v {
  font:8pt Arial, Helvetica, sans-serif;
...
.disc-v {
  color:#c00; font:bold 8pt Arial, Helvetica, sans-serif;
  text-align:right; vertical-align:middle;
  padding:7pt 12pt 7pt 6pt;
}
...
`):""}${p?`
  ...
  <div class="contract-page">
    ...
    <div class="contract-sigs-flow">
      ...
      <div style="margin-top:14pt;font:6pt Arial, Helvetica, sans-serif;">Testemunhas:</div>
