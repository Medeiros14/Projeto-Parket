...
(function () {
  ...
  function refreshKanbanBadges() {
    ...
    var topics = Object.keys(unreadByTopic).filter(function (t) { return unreadByTopic[t] > 0; });
    if (!topics.length) return;
    var cards = document.querySelectorAll('div[class*="card"],article,div[class*="Card"]');
    var seen = new Set();
    cards.forEach(function (el) {
      if (seen.has(el)) return;
      var tx = (el.textContent || "").slice(0, 800);
      for (var i = 0; i < topics.length; i++) {
        var topic = topics[i];
        if (topic.length < 3) continue;
        if (tx.indexOf(topic) === -1) continue;
        var node = el;
        for (var d = 0; d < 4; d++) {
          if (!node || !node.getBoundingClientRect) break;
          var rect = node.getBoundingClientRect();
          if (rect.height > 0 && rect.height < 260 && rect.width > 100 && rect.width < 520) {
            if (node.querySelector(".__pkt-chat-badge")) break;
            if (getComputedStyle(node).position === "static") node.style.position = "relative";
            var b = document.createElement("div");
            b.className = "__pkt-chat-badge";
            b.title = "Nova mensagem no chat — clique no card pra abrir";
            b.style.cssText = "position:absolute;top:6px;right:6px;background:#EF4444;color:#fff;font-size:10px;font-weight:800;padding:2px 7px;border-radius:10px;line-height:1.3;box-shadow:0 2px 8px rgba(239,68,68,0.6);z-index:10;pointer-events:none;display:flex;align-items:center;gap:3px;";
            b.innerHTML = '<span style="font-size:9px">💬</span>' + unreadByTopic[topic];
            node.appendChild(b);
            seen.add(node);
            break;
          }
  ...
  async function pollNewMessages() {
    var me = ckUserEmail();
    if (!me) return;
    var since = _lastPollTs || new Date(Date.now() - 24 * 3600 * 1000).toISOString();
    try {
      var url = SUPA_URL + "/rest/v1/card_chat_messages?created_at=gt." + encodeURIComponent(since) +
                "&user_email=not.eq." + encodeURIComponent(me) +
                "&select=*&order=created_at.asc&limit=50";
      ...
      rows.forEach(function (m) {
        ...
        if (!unreadByContext[cid]) {
          showToast(m, cid);
          unreadByContext[cid] = { count: 1, topic: m.topic, lastMsg: m };
        } else {
          unreadByContext[cid].count++;
          unreadByContext[cid].lastMsg = m;
        }
        if (m.topic) unreadByTopic[m.topic] = (unreadByTopic[m.topic] || 0) + 1;
      });
  ...
  setInterval(pollNewMessages, 12000);
  setInterval(refreshKanbanBadges, 2500);
